...
void cudaPolygon::fillRectangle()
{
        ...
        printf( "cudaPolygon::fillRectangle: Filling x0 = %d , y0 = %d , x1 = %d , y1 = %d , size = %d , sector = %d\n",
...
{
    /**
      Method is used to make a host copy of the deformed points for plotting purposes
      */
#if DEBUG_CUDA_POLYGON
    printf( "cudaPolygon::getDefXY0ToCPU\n" );
#endif

    // Generate deformed points for the level 0 as a copy of the undeformed parameters
...
CorrelationResult *cudaPolygon::getCorrelationResultsToCPU()
{
    ...
    cudaMemcpy( cpuCorrelationResults,
                gpuCorrelationResults,
                sizeof( CorrelationResult ),
                cudaMemcpyDeviceToHost );

    // Number of points comes from the thrust device_vector, which has its size in the cpu
...
float *cudaPolygon::getGlobalABChi()
{
    return globalABChi;
}
...
{
    if ( level == currentPyramidLevel ) return;
   ...
   (
        parameters[ parType_lastGood ],
        parameters[ parType_tentative ],
        parameters[ parType_saved ],
        fittingModel,
        multiplier
   );
...
void cudaPolygon::initializeParametersLevel0( float *initialGuess_ )
{
    ...
    cudaMemcpy( parameters[ parType_lastGood  ],
                initialGuess_,
                numberOfModelParameters* sizeof( float ),
...
void cudaPolygon::updatePolygon( deformationDescriptionEnum deformationDescription )
{
#if DEBUG_CUDA_POLYGON
...
void cudaPolygon::transferParameters( parameterTypeEnum parSrc , parameterTypeEnum parDst )
{
    if ( parDst == parSrc ) return;

    cudaMemcpy( parameters[ parDst ],
                parameters[ parSrc ],
                numberOfModelParameters * sizeof( float ),
                cudaMemcpyDeviceToDevice );
...
void cudaPolygon::makeAllUndLevels()
{
    ...
    for ( int ilevel = firstLevel ; ilevel <= stop ; ilevel += step )
    {
        ...
        (
            thrust::make_zip_iterator( thrust::make_tuple( undeformed_xs[ prevLevel ].begin() , undeformed_ys[ prevLevel ].begin() ) ),
            thrust::make_zip_iterator( thrust::make_tuple( undeformed_xs[ prevLevel ].end()   , undeformed_ys[ prevLevel ].end()   ) ),
            thrust::make_zip_iterator( thrust::make_tuple( undeformed_xs[ ilevel ].begin()    , undeformed_ys[ ilevel ].begin()    ) ),
        ...
        (
            thrust::make_zip_iterator( thrust::make_tuple( undeformed_xs[ ilevel ].begin() , undeformed_ys[ ilevel ].begin() ) ),
            thrust::make_zip_iterator( thrust::make_tuple( undeformed_xs[ ilevel ].end()   , undeformed_ys[ ilevel ].end()   ) ),
...
void cudaPolygon::allocateGlobalABChi()
{
    ...
    int numberOfBlocks = ( numberOfPoints + THREADS_PER_BLOCK - 1 ) / THREADS_PER_BLOCK;
    int globalSize =    sizeof( float ) *
                        numberOfBlocks *
                        ( 1 + numberOfModelParameters * ( 1 + numberOfModelParameters ) );

    if ( globalABChi ) deallocateGlobalABChi();
...
void cudaPolygonAnnular::cleanAnnularRectangle0( float r , float dr , float a , float da,
...
void cudaPolygonBlob::cleanBlobRectangle0( v_points blobContour )
{
    ...
    (
        ...
        thrust::make_zip_iterator( thrust::make_tuple( undeformed_xs[ 0 ].begin() , undeformed_ys[ 0 ].begin() ) ),
        thrust::make_zip_iterator( thrust::make_tuple( undeformed_xs[ 0 ].end()   , undeformed_ys[ 0 ].end()   ) ),
        removeBlobFunctor( contours.data() , lineEquations.data() , lineEquations.size()  )
    );
