...
class managerClass : public QObject
{
    ...
    void set_deformed_image                             ( std::string def_fileName_in ,
                                                          int current_frame_order );
    ...
    bool perform_single_frame_correlation_blob          ( frame_results* results_in           , int current_frame_order );

    void adjust_rectangular_domain                      ( int &center_x, int &center_y,
                                                          frame_results* previous_results     , int current_frame_order );

    void adjust_annular_domain                          ( int i, int j, float &r, float &a,
                                                          float &ri, float dr, float da, float &center_x, float &center_y, int as,
                                                          frame_results* all_previous_results , int current_frame_order );

    void adjust_blob_domain                             ( frame_results* previous_results     , int current_frame_order );
    ...
    v_points get_inside_points_blobDomain_non_convex    ( v_points xy_contour_in, int max_x, int min_x, int max_y, int min_y );
    v_points get_inside_points_blobDomain_convex        ( v_points xy_contour_in, int max_x, int min_x, int max_y, int min_y );
    ...
    v_points get_contour_points_annularDomain           ( float r, float dr, float a, float da,
                                                          float cx, float cy );
    ...
    intersectionEnum check_segment_intersection         ( float v1x1, float v1y1, float v1x2, float v1y2,
                                                          float v2x1, float v2y1, float v2x2, float v2y2 );

    intersectionEnum check_segment_intersection         ( float v1x2, float v1y2,
                                                          float v2y1, float v2y2,
                                                          const std::vector< float > &lineEquation );


    void update_results                                 ( frame_results* results_in , CorrelationResult *correlationResult = nullptr );
    void update_global_results                          ( int hrs, int vas, frame_results* results_in );
    void addFrameToReport                               ( int frame_number_in, int results_i_in, int results_j_in, frame_results* results_in );
...
public:
    ...
    void set_deformation_description                    ( deformationDescriptionEnum deformation_description_in );
    ...
    void set_pyramid                                    ( int pyramid_start_in , int pyramid_step_in , int pyramid_stop_in );
...
private slots:
    void perform_multiframe_correlation                 ();                              /**< Private method can only be called via "emit" so that it runs on the manager thread */
};
