...
protected:
    ...
    float *und_xy_positions                 = nullptr;  //Array with all the undeformed locations for all points
    float *und_intensities                  = nullptr;  //Array with all the undeformed intensities for all points - has to go
    float *w_results                        = nullptr;  //Array with all the deformed intensities for all points - has to go
    float *dwdxy_results                    = nullptr;  //Array with all the deformed intensities gradients for all points - has to go
    float *def_xy_positions                 = nullptr;  //Array with all the deformed locations for all points - has to go

    float *und_w                            = nullptr; // array with undeformed intensity for one point - one entry per color
    float *def_w                            = nullptr; // array with deformed intensity for one point - one entry per color
    float *def_dwdxy                        = nullptr; // array with deformed intensity gradients (wrt x,y) for one point - two entries per color
    float *dTxydp                           = nullptr; // array with transformation gradients (wrt p) for one point - two entries per number_of_parameters
    ...
    virtual void get_new_interpolation_parameters( float *part_of_all_parameters, int x_pos, int y_pos, int color_in ) = 0;
    virtual void get_interpolation( float *part_of_w_results, float *part_of_dwdxy_results, float xdef, float ydef ) = 0;
...
public:		
    ...
    void set_def_image( unsigned char* def_image_ptr_in,
                        int def_image_rows_in,
                        int def_image_cols_in,
                        int def_image_step_in,
                        float *all_parameters_in );
    void set_und_image( unsigned char* und_image_ptr_in,
    ...
    static int get_number_of_interpolation_parameters( interpolationModelEnum interpolationModel_in );
    static int get_number_of_model_parameters( fittingModelEnum fittingModel_in );
    static std::vector < InterpolationClass* > new_InterpolationClass(
...
protected:
    inline void get_new_interpolation_parameters( float *part_of_all_parameters, int x_pos, int y_pos, int color_in );
    inline void get_interpolation( float *part_of_w_results, float *part_of_dwdxy_results, float xdef, float ydef );

public:
    InterpolationClass_bicubic( const int number_of_colors_in,
                                const fittingModelEnum &fittingModel_in ) :
                                    InterpolationClass( number_of_colors_in ,
                                                        fittingModel_in )
    {
        interpolation_model = im_bicubic;
        number_of_interpolation_parameters = get_number_of_interpolation_parameters( interpolation_model );
