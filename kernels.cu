...
{
    int size_A_B = number_of_model_parameters * number_of_model_parameters +  number_of_model_parameters;
    ...
    for ( int j = 0 ; j < number_of_model_parameters ; ++j )
    {
        ...
        global_mat_A_and_vec_B_and_CHI[ j * number_of_model_parameters + j ] *= ( 1.f + lambda );

        //  Copy upper part of the matrix to the lower part ( already done in Cholesky? )
        //for ( int i = 0; i < j; ++i )
        //{
        //    global_mat_A_and_vec_B_and_CHI[ j * number_of_model_parameters + i ] = global_mat_A_and_vec_B_and_CHI[ i * number_of_model_parameters + j ];
        //}
    }
}
...
{
    int size_A_B_CHI = number_of_model_parameters * number_of_model_parameters + number_of_model_parameters + 1;

    for ( int j = 0 ; j < size_A_B_CHI ; ++j )
    {
        // Copy vec_B
        global_mat_A_and_vec_B_and_CHI [ j ] += global_mat_A_and_vec_B_and_CHI [ j + size_A_B_CHI ];
    }
}
    ...
    const int number_of_model_parameters )
{
    ...
    // Contribution to global_mat_A_and_vec_B_and_CHI from block "bid" is already in global memory.
    int tid = threadIdx.x;
    int bid = blockIdx.x;
    ...
    {        
        ...
        memcpy(
                    &shared_mat_A_and_vec_B_and_CHI[ tid * size_A_B_chi ],
                    &global_mat_A_and_vec_B_and_CHI[ ( bid * blockDim.x + tid ) * size_A_B_chi ],
                     size_A_B_chi * sizeof( float )
              );

        __syncthreads();

        // Do a layer of reduction in shared memory. Up to 256 threads from this block will reduce to 1
        for ( int s = 1 ; s < blockDim.x ; s *= 2 )
        {

            if ( ( tid % ( 2 * s ) ) == 0 && ( tid + s ) + blockDim.x * blockIdx.x < number_of_blocks )//don't include contribution from non-existing blocks
            {
        ...
        // Thread id = 0 includes the contribution of its block's shared ABchi into the global space for another layer of global reduction
        if ( tid == 0 )
        {
            memcpy(
                        &global_mat_A_and_vec_B_and_CHI[ bid * size_A_B_chi ],
          ...
          errorEnum *error_code )
{
    ...
    if (i < number_of_points)
    {
        ...
        d_def_xy_positions[ 2 * i +  0 ] = und_x + (float)d_parameters[0] - dy * (float)d_parameters[2];
        d_def_xy_positions[ 2 * i +  1 ] = und_y + (float)d_parameters[1] + dx * (float)d_parameters[2];
          ...
          errorEnum *error_code )
{
    ...
    if ( i < number_of_points )
    {
        ...
        d_def_xy_positions[ 2 * i +  0 ] = und_x + (float)d_parameters[0] + dx * (float)d_parameters[2] + dy * (float)d_parameters[3];
        d_def_xy_positions[ 2 * i +  1 ] = und_y + (float)d_parameters[1] + dx * (float)d_parameters[4] + dy * (float)d_parameters[5];
    ...
    errorEnum *error_code )
{
    ...
    {			
        ...
        for ( int c = 0; c < number_of_colors; ++c )
        {
            ...
            float def_w    = (float) tex2D<unsigned char>( def_tex , ( number_of_colors * ix0 + c ) , iy0 );

            //  gradient dw/dx of the Interpolated deformed image
            float def_dwdx = (float) tex2D<unsigned char>( def_tex , ( number_of_colors * ix1 + c ) , iy0 ) -
                             (float) tex2D<unsigned char>( def_tex , ( number_of_colors * ix0 + c ) , iy0 );

            //  gradient dw/dy of the Interpolated deformed image
            float def_dwdy = (float) tex2D<unsigned char>( def_tex , ( number_of_colors * ix0 + c ) , iy1 ) -
                             (float) tex2D<unsigned char>( def_tex , ( number_of_colors * ix0 + c ) , iy0 );
        ...
        {
            if ( ( tid % ( 2 * s ) ) == 0 && ( tid + s ) + blockDim.x * blockIdx.x < number_of_points ) //don't include contribution from the last block
            {
                for ( int index = 0 ; index < size_A_B_chi ; ++index )
                    shared_mat_A_and_vec_B_and_CHI[ tid * size_A_B_chi + index ] += shared_mat_A_and_vec_B_and_CHI[ ( tid + s ) * size_A_B_chi + index ];
            }

            __syncthreads();
        }

        // Thread id = 0 includes the contribution of its block into the global space for later global reduction
        if ( tid == 0 )
        {
            for ( unsigned int index = 0 ; index < size_A_B_chi ; ++index )
                global_mat_A_and_vec_B_and_CHI[ bid * size_A_B_chi + index ] = shared_mat_A_and_vec_B_and_CHI[ index ];
        }
    ...
    if ( i < number_of_points )
    {
        ...
        for ( int c = 0 ; c < number_of_colors ; ++c )
        {
            ...
            float all_parameters[ 4 ] = {
                                        ...
                                        (float)tex2D <unsigned char>( def_tex , ( number_of_colors * ix1 + c ) , iy0 ) -
                                        (float)tex2D <unsigned char>( def_tex , ( number_of_colors * ix0 + c ) , iy0 ),

                                        (float)tex2D <unsigned char>( def_tex , ( number_of_colors * ix0 + c ) , iy1 ) -
                                        (float)tex2D <unsigned char>( def_tex , ( number_of_colors * ix0 + c ) , iy0 ),

                                        (float)tex2D <unsigned char>( def_tex , ( number_of_colors * ix1 + c ) , iy1 ) -
                                        (float)tex2D <unsigned char>( def_tex , ( number_of_colors * ix1 + c ) , iy0 ) -
                                        (float)tex2D <unsigned char>( def_tex , ( number_of_colors * ix0 + c ) , iy1 ) +
                                        (float)tex2D <unsigned char>( def_tex , ( number_of_colors * ix0 + c ) , iy0 )
                                                                     };
...
)
{
    ...
    if ( iDst < dstCols && jDst < dstRows )
    {
        ...
        unsigned char pyramid = (unsigned char)
        (
            0.0025f * (float) tex2D<unsigned char>( src, iSrc0 , jSrc0 ) +
            0.0125f * (float) tex2D<unsigned char>( src, iSrc1 , jSrc0 ) +
            0.0200f * (float) tex2D<unsigned char>( src, iSrc2 , jSrc0 ) +
            0.0125f * (float) tex2D<unsigned char>( src, iSrc3 , jSrc0 ) +
            0.0025f * (float) tex2D<unsigned char>( src, iSrc4 , jSrc0 ) +

            0.0125f * (float) tex2D<unsigned char>( src, iSrc0 , jSrc1 ) +
            0.0625f * (float) tex2D<unsigned char>( src, iSrc1 , jSrc1 ) +
            0.1000f * (float) tex2D<unsigned char>( src, iSrc2 , jSrc1 ) +
            0.0625f * (float) tex2D<unsigned char>( src, iSrc3 , jSrc1 ) +
            0.0125f * (float) tex2D<unsigned char>( src, iSrc4 , jSrc1 ) +

            0.0200f * (float) tex2D<unsigned char>( src, iSrc0 , jSrc2 ) +
            0.1000f * (float) tex2D<unsigned char>( src, iSrc1 , jSrc2 ) +
            0.1600f * (float) tex2D<unsigned char>( src, iSrc2 , jSrc2 ) +
            0.1000f * (float) tex2D<unsigned char>( src, iSrc3 , jSrc2 ) +
            0.0200f * (float) tex2D<unsigned char>( src, iSrc4 , jSrc2 ) +

            0.0125f * (float) tex2D<unsigned char>( src, iSrc0 , jSrc3 ) +
            0.0625f * (float) tex2D<unsigned char>( src, iSrc1 , jSrc3 ) +
            0.1000f * (float) tex2D<unsigned char>( src, iSrc2 , jSrc3 ) +
            0.0625f * (float) tex2D<unsigned char>( src, iSrc3 , jSrc3 ) +
            0.0125f * (float) tex2D<unsigned char>( src, iSrc4 , jSrc3 ) +

            0.0025f * (float) tex2D<unsigned char>( src, iSrc0 , jSrc4 ) +
            0.0125f * (float) tex2D<unsigned char>( src, iSrc1 , jSrc4 ) +
            0.0200f * (float) tex2D<unsigned char>( src, iSrc2 , jSrc4 ) +
            0.0125f * (float) tex2D<unsigned char>( src, iSrc3 , jSrc4 ) +
            0.0025f * (float) tex2D<unsigned char>( src, iSrc4 , jSrc4 )
        );
