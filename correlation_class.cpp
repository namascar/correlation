...
//  3) Assembler puts togeter the thread contribution to the mat_A and vec_B -coming soon
//void *h_interpolation( void *correlation_thread_data_in )
s_correlation_thread_data h_interpolation( s_correlation_thread_data thread_data )
{
...
void CorrelationClass::initialize_correlation()
{
    ...
    // Factory implementation to create the selected interpolation objects, one per thread
    interpolators = InterpolationClass::new_InterpolationClass( interpolationModel,
                                                                fittingModel,
                                                                number_of_colors,
                                                                number_of_threads);

    // Factory implementation to create the selected model objects, one per thread
    modelers = ModelClass::new_ModelClass( fittingModel,
                                           number_of_threads );
}
...
CorrelationClass::~CorrelationClass()
{
    ...
    for ( int i = 0 ; i < number_of_threads ; ++i ) // interpolators is a vector of dynamically allocated InterpolationClass
    {
        delete interpolators[ i ];
    }
}

void CorrelationClass::apply_model_and_interpolate( int pyramid_level , bool runInterpolation )
{
    ...
    cv::Mat undTemp( und_image_rows , und_image_cols , CV_8U , und_image_ptr , und_image_step );
    cv::imshow( "und image" , undTemp );

    cv::Mat defTemp( def_image_rows , def_image_cols , CV_8U , def_image_ptr , def_image_step );
    ...
    for ( int ithread = 0;  ithread < number_of_threads; ++ithread )
    {
        ...
        correlation_thread_data[ ithread ].number_of_points_this_thread = points_per_thread;
    ...
    // Left over points
    for ( int ithread = 0;  ithread < number_of_points % number_of_threads; ++ithread )
    {
    ...
    for ( int ithread = 0 ;  ithread < number_of_threads ;  ++ithread )
	{
        ...
        correlation_thread_data[ ithread ].first_point_this_thread = first_point_this_thread;

		// Divide the transformation problem and the interpolation problem for the modeleres and interpolators.
        // 	Every thread gets a reference to the corresponding entry of the und_xy_positions in the undeformed image
		//	which is the thread number multiplied by the number of points in the threads, times 2 (x and y).
		//	This way, for example, if there are 15 points per thread, the reference of thread 0th, "points" at the 
		//	location 0, the next thread has a reference pointing to the entry 15 * 2  = 30. The last thread may have
		//	more items than the rest, but the begining of the array follows the same rule. Similar rule applies for the 
		//	def_xy_points, w_results and dwdxy_results.
        // 	NOTE THIS IS JUST SETTING THE PROBLEMS FOR EACH THREAD, NOT SOLVING THEM - that is done by h_interpolation

        modelers[ ithread ].set_points(  correlation_thread_data[ ithread ].number_of_points_this_thread, // this object needs to go away
                                        &und_xy_positions       [ first_point_this_thread * 2 ],
                                        &def_xy_positions       [ first_point_this_thread * 2 ],
										model_parameters,
										und_x_center,
										und_y_center,
                                        &dTxydp                 [ first_point_this_thread  *
                                                                  number_of_model_parameters * 2 ] );
        if ( runInterpolation )
        {
            interpolators[ ithread ]->set_und_image( und_image_ptr,
                                                     und_image_step  );

            interpolators[ ithread ]->set_def_image( def_image_ptr,
                                                     def_image_rows,
                                                     def_image_cols,
                                                     def_image_step,
                                                     all_interpolation_parameters );

            interpolators[ithread]->set_multiple_interpolations(      correlation_thread_data[ ithread ].number_of_points_this_thread,
                                                                     &und_intensities        [ first_point_this_thread * number_of_colors ], // this needs to go away
                                                                     &und_xy_positions       [ first_point_this_thread * 2 ],
                                                                     &def_xy_positions       [ first_point_this_thread * 2 ], // this needs to go away
                                                                     &w_results              [ first_point_this_thread * number_of_colors ], // this needs to go away
                                                                     &dwdxy_results          [ first_point_this_thread * number_of_colors * 2 ], // this needs to go away
                                                                      model_parameters,
                                                                      und_x_center,
                                                                      und_y_center,
                                                                     &dTxydp                 [ first_point_this_thread  *
        ...
        correlationThreads.push_back( std::async( std::launch::async ,  h_interpolation , correlation_thread_data[ ithread ] ) );

        first_point_this_thread += correlation_thread_data[ ithread ].number_of_points_this_thread;
	}

	//join threads
    for ( int ithread = 0; ithread < number_of_threads; ithread++ )
	{
        s_correlation_thread_data correlationResult = correlationThreads[ ithread ].get();
        ...
        if ( runInterpolation )
        {
            ...
            //  Aggregates contributions from all threads to construct global chi, mat_A and vec_B
            ...
            for ( int p1 = 0; p1 < number_of_model_parameters; ++p1 )
            {
                ...
                for ( int p2 = p1; p2 < number_of_model_parameters; ++p2 )
                    mat_A [ p1 * number_of_model_parameters + p2 ] +=
                        correlationResult.contribution_mat_A[  p1 * number_of_model_parameters + p2  ];
            }
        }

        if( correlationResult.error_status ) error_code = correlationResult.error_code;
        error_status = error_status || correlationResult.error_status;
		
		#if DEBUG_THREAD
			std::cout << "correlation() : completed thread id : " << ithread << "  exiting with status : " << status << std::endl;
		#endif
	}

    #if DEBUG_TIME_MODEL_AND_INTERPOLATION
        auto duration_model_and_interpolation = ( std::clock() - start_model_and_interpolation ) / (float) CLOCKS_PER_SEC;
        std::cout<<"correlation() :model and interpolation execution time(s): "<< duration_model_and_interpolation <<'\n';
	#endif
...
float *CorrelationClass::get_model_parameters()
{
                                            ...
                                            float *und_xy_positions_in )
{
    ...
    return CorrelationClass::Newton_Raphson(    model_parameters_in,
                                                number_of_points_in,
                                                und_xy_positions_in );
}

// This method is for correlation of a new set of points, maybe with a different number
// of points than already allocated. We will delete the old und_xy_positions, def_xy_positions
// w_results, dwdxy_results and dTxydp arrays and reallocate new ones if the sizes are different.
...
//This method assumes no changes on the correlation set, so und_xy_positions will
// still be used. Only change is the initial guess, passed as model_parameters_in
float *CorrelationClass::Newton_Raphson( float *model_parameters_in )
{
    //start timing
    ...
    //  New parameters and chi are computed in the same loop since there is a lot of sinergy.
    ...
    for ( int pyramid_level = pyramid_stop ; pyramid_level >= pyramid_start ; pyramid_level -= pyramid_step )
    {
        pyramid.translate_model_parameters( model_parameters , pyramid_level_old , pyramid_level );
        ...
        last_good_chi = chi ;
            ...
            if ( last_good_chi > 1e8 ) Newton_Raphson_dump( pyramid_level);
            //return model_parameters;
        #endif

        //   Since we have the matrix_A and vector_B contributions from the chi calculation,
        ...
        for ( int iteration = 1; iteration <= maximum_iterations + 1 ; ++iteration )
        {
            ...
            // Sequencial computation of model_parameters and chi has a lot of sinergy. The new
            //      model_parameters can be computed at the same time as chi, but they would be
            //      the future parameter set. Instead of running the multithreading engine twice,
            //      the future parameter set is saved and reused, only if the chi in last step was better.
            ...
            //  Calls multithreading engine to assemble upper half of the correlation matrix for this iteration
            ...
            else
            {
                ...
                for ( int p = 0; p < number_of_model_parameters; ++p ) //These are the new set of model_parameters
                {
                    tentative_model_parameters[ p ] = model_parameters[ p ];
                }
            }

            //  Computation of CHI FOR THE TENTATIVE set of model parameters. ( put them in "model_parameteres" )
            for ( int p = 0; p < number_of_model_parameters; ++p )
            {
            ...
            //   Since we have the matrix_A and vector_B contributions from the chi calculation,
            //      may as well compute the next parameters and save them for the likely case
            ...
            //  Compares delta chi based on last_good_chi(last_good_model_parameters) and
            //      chi(model_parameters). However, it does not act on this info until the
            //      parameters are updated
            float delta_chi = std::abs( (last_good_chi - chi) / ( std::max(last_good_chi,chi) + required_precision ) );
                ...
                printf("tent. chi:%12.4e :delta chi: %12.4e lambda: %12.4e ", chi, delta_chi, lambda);
            #endif

            if ( chi <= last_good_chi ) //converging step - record the new "best" parameters
            {
                last_good_chi = chi;
            ...
            else	//diverging step - increase lambda and keep last good set of parameters
            {
    ...
    duration_newton_raphson = ( std::clock() - start_newton_raphson ) / (float) CLOCKS_PER_SEC;
    #if DEBUG_TIME_NEWTON_RAPHSON
        std::cout<<"correlation() :Newton_Raphson execution time(s): "<< duration_newton_raphson<<'\n';
	#endif

    #if DEBUG_TIME_ALL_MODEL
        std::cout<<"correlation() :ALL model execution time(s): "<< time_all_model << " for n = " << pyramid.get_number_of_points( 0 ) << " and " << number_of_threads << " threads" << " iterations = " << reached_iterations << '\n';
    #endif

    #if DEBUG_TIME_ALL_INTERPOLATION
        std::cout<<"correlation() :ALL interpolation execution time(s): "<< time_all_interpolation << " for n = " << pyramid.get_number_of_points( 0 ) << " and " << number_of_threads << " threads" << " iterations = " << reached_iterations << '\n';
    #endif

    #if DEBUG_TIME_ALL_NEW_PARAMETERS_ASSEMBLY
        std::cout<<"correlation() :ALL matrix assembly execution time(s): "<< time_all_new_parameters_assembly << " for n = " << pyramid.get_number_of_points( 0 ) << " and " << number_of_threads << " threads" << " iterations = " << reached_iterations << '\n';
    #endif

    #if DEBUG_TIME_ALL_NEW_PARAMETERS_SOLVER
        std::cout<<"correlation() :ALL matrix solver execution time(s): "<< time_all_new_parameters_solver << " for n = " << pyramid.get_number_of_points( 0 ) << " and " << number_of_threads << " threads" << " iterations = " << reached_iterations << '\n';
    #endif
...
void CorrelationClass::compute_model_parameters( float lambda , float scaling )
{
    ...
    //  Hessian matrix is symetric. Only upper part was computed, now copy to lower part.
    //  p1 index starts with 0 intentionally, to apply the LM scalling to mat_A[ 0, 0 ]
    for ( int p1 = 0; p1 < number_of_model_parameters; ++p1 )
    {
		// this loop will not be executed the first time, that is ok
        for ( int p2 = 0; p2 < p1; ++p2 )
			mat_A [ p1 * (number_of_model_parameters) + p2 ] = mat_A [ p2 * (number_of_model_parameters) + p1 ];

        //  Apply the Lebevenberg-Marquardt scaling to the diagonal
        mat_A [ p1 * (number_of_model_parameters) + p1 ] *= ( 1.f + lambda );
    }

    duration_new_parameters_assembly = ( std::clock() - start_new_parameters_assembly ) / (float) CLOCKS_PER_SEC;
    time_all_new_parameters_assembly += duration_new_parameters_assembly;
    #if DEBUG_TIME_NEW_PARAMETERS_ASSEMBLY
        std::cout<<"correlation() :new parameters assembly execution time(s): "<< duration_new_parameters_assembly <<'\n';
    #endif
    ...
    //**********************************************************************************
    //   Solve the system
    float *dp = solve(); //dp now points to the solution[number_of_model_parameters] working space
	//**********************************************************************************

    for ( int p = 0; p < number_of_model_parameters; ++p )
        model_parameters[ p ] += dp[ p ];

    duration_new_parameters_solver = ( std::clock() - start_new_parameters_solver ) / (float) CLOCKS_PER_SEC;
    time_all_new_parameters_solver += duration_new_parameters_solver;
    #if DEBUG_TIME_NEW_PARAMETERS_SOLVER
        std::cout<<"correlation() :new parameters solver execution time(s): "<< duration_new_parameters_solver <<'\n';
    #endif

    duration_new_parameters = ( std::clock() - start_new_parameters ) / (float) CLOCKS_PER_SEC;
    #if DEBUG_TIME_NEW_PARAMETERS
		std::cout<<"correlation() :new parameters execution time(s): "<< duration_new_parameters <<'\n';
    #endif
}

bool CorrelationClass::get_error_status()
{
	return error_status;
}

errorEnum CorrelationClass::get_error_code()
{
	return error_code;
}
...
float *CorrelationClass::solve()
{
	...
	//		//opencv solver
	//    cv::Mat A = cv::Mat(number_of_model_parameters, number_of_model_parameters ,CV_32FC1, mat_A);
	...
	Eigen::Map<Eigen::MatrixXf> eigen_map_A(mat_A, number_of_model_parameters, number_of_model_parameters);
    ...
    for ( int p = 0; p < number_of_model_parameters; ++p )
		...
		std::cout << std::endl << " residuals " << std::endl << eigen_map_A * eigen_map_x - eigen_map_B << std::endl;
		std::cout << std::endl;
...
void CorrelationClass::debug_correlation_info( int pyramid_level )
{
        ...
        std::cout << "number of interpolation parameters " << number_of_interpolation_parameters << std::endl;
        std::cout << "fitting model " << fittingModel << std::endl;
        std::cout << "number of model parameters " << number_of_model_parameters << std::endl;
...
void CorrelationClass::debug_correlation( int pyramid_level )
{
        ...
        for ( int ipoint = 0; ipoint < number_of_points; ++ipoint )
        {
            ...
            for ( int c = 0; c < number_of_colors; ++c )
                std::cout << (float) und_image_ptr[ und_image.step1() * y + x * number_of_colors + c] << ", ";
            ...
            std::cout << " dwdy = ";
            for ( int c = 0; c < number_of_colors; ++c )
                std::cout << dwdxy_results[ipoint * (number_of_colors * 2) + number_of_colors + c] << ", ";  
...
float CorrelationClass::get_chi()
{
    return last_good_chi;
}
...
int CorrelationClass::get_iterations()
{
...
v_points CorrelationClass::getDefXY0()
{
    ...
    memcpy( defXY.data() ,
            def_xy_positions ,
            numberOfPoints0 * 2 * sizeof( float ) );
...
void CorrelationClass::Newton_Raphson_dump( int pyramid_level )
{
    ...
    printf("                            point       und_x       und_y       und_w       def_x       def_y       def_w       def_wx      def_wy\n");

    for ( int ipoint = 0; ipoint < number_of_points; ++ipoint )
    {
        if ( abs ( w_results[ ipoint * number_of_colors ] )  > 1000 )
        {
            printf(" correlation:           %6d     %10.4f   %10.4f   %10.4f   %10.4f   %10.4f   %10.4f   %10.4f   %10.4f\n",
                   ipoint,
                   und_xy_positions[ ipoint * 2     ],
                   und_xy_positions[ ipoint * 2 + 1 ],
                   und_intensities [ ipoint * number_of_colors ],
                   def_xy_positions[ ipoint * 2     ],
                   def_xy_positions[ ipoint * 2 + 1 ],
                   w_results       [ ipoint * number_of_colors ],
                   dwdxy_results   [ ipoint * 2     ],
            ...
            int index0 = ( ix + iy * def_image.cols ) * ( number_of_colors * number_of_interpolation_parameters + 1 ) + 1;
            ...
            for ( int c = 0; c < number_of_colors; c++ )
            {
                ...
                for ( int jk = 0; jk < 4; jk++ )
                {
                    ...
                    for ( int ik = 0; ik < 4; ik++ )
                    {
                        int index_c_jk_ik = index_c_jk + ik;
                                   ...
                                   //   w_result
                           debug_w_results     [                    c ] +=      all_interpolation_parameters[ index_c_jk_ik ] * py[ jk     ]  *  px[ ik     ];

                        if ( ik > 0 ) //dwdx_result
                           debug_dwdxy_results [                    c ] += ik * all_interpolation_parameters[ index_c_jk_ik ] * py[ jk     ]  *  px[ ik - 1 ];

                        if ( jk > 0 ) //dwdy_result
                           debug_dwdxy_results [ number_of_colors + c ] += jk * all_interpolation_parameters[ index_c_jk_ik ] * py[ jk - 1 ]  *  px[ ik     ];
                ...
                printf("\n xdef=%10.4f      ydef=%10.4f      w=%10.4f      wx=%10.4f      wy%10.4f      \n", xdef, ydef, debug_w_results[ c ], debug_dwdxy_results[ c ], debug_dwdxy_results[ number_of_colors + c ]    );
            ...
            for ( int ithread = 0; ithread < number_of_threads; ++ ithread )
            {
                ...
                for ( int j = 0; j < correlation_thread_data[ ithread ].number_of_points_this_thread; ++j )
                {
                    int ipoint = correlation_thread_data[ ithread ].first_point_this_thread + j;

                    printf(" thread = %2d, xdef = %10.4f, ydef = %10.4f\n", ithread, def_xy_positions[ ipoint * 2     ], def_xy_positions[ ipoint * 2 + 1 ] );
                }
            }
