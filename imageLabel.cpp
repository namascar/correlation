...
void ImageLabel::mousePressEvent(QMouseEvent *event) {
  ...
  if (selecting_ic && selecting && event->button() == Qt::LeftButton) {
    ...
    case fm_UVQ:
      if (domain == domain_annular) {
        ...
        if (ic_und_x > def_hi_left_x && ic_und_x < def_lo_right_x &&
          ...
          UVUxUyVxVyIC = UVUxUyVxVyIC_Center;
        } else if (abs(ic_und_x - def_center_x) >
                   abs(ic_und_y - def_center_y)) {
    ...
    // connect(def_imageLabel, SIGNAL( stop_correlation_display() ), this, SLOT(
    // stop_correlation_display() ));
  ...
  // We are selecting a domain
    ...
    //  Check if we are adjusting the position of the intial domain or creating
    //  a new one
    //      Clicking inside an existing domain modifies it, clicking outside
    //      creates a new
    //      one.
    adjusting_domain = false;
    ...
    case domain_rectangular: {
      if (r < 0.5f * std::min((int)abs(right_x - left_x),
                              (int)abs(right_y - left_y))) {
        adjusting_domain = true;
      }
    } break;

    case domain_blob: {
      if (r < 0.5f * std::min((int)abs(blob_x_max - blob_x_min),
                              (int)abs(blob_y_max - blob_y_min))) {
        adjusting_domain = true;
      }
    } break;
    ...
    } else // Make a new domain selection if clicking away from the center
    {
      ...
      case domain_annular:
        ...
        emit valueChanged_selectingAnnular(center_x, center_y, inside_radius,
                                           outside_radius, ri_by_ro,
      ...
      case domain_rectangular:
        ...
        emit valueChanged_selectingRectangular(center_x, center_y, right_x,
      ...
      case domain_blob: {
        ...
        std::pair<float, float> xy_pair =
            std::make_pair((float)event->x(), (float)event->y());
        xy_blob.push_back(xy_pair);

        // Calls mirrorSelectionBlob on the def_imageLabel
        emit valueChanged_selectingBlob(center_x, center_y, xy_blob,
                                        clickScale_x, clickScale_y);
...
void ImageLabel::mouseMoveEvent(QMouseEvent *event) {
  ...
  if (selecting_ic && selecting) {
    ...
    case fm_U:
      ...
      model_parameters[0] =
          prev_model_parameters[0] + (ic_def_x - ic_und_x) / realScale_x;
    ...
    case fm_UV:
      ...
      model_parameters[0] =
          prev_model_parameters[0] + (ic_def_x - ic_und_x) / realScale_x;
      model_parameters[1] =
          prev_model_parameters[1] + (ic_def_y - ic_und_y) / realScale_y;
    ...
    case fm_UVQ:
      ...
      case UVUxUyVxVyIC_DU:
        ...
        model_parameters[2] = prev_model_parameters[2] +
                              (ic_def_y - ic_und_y) *
                                  (ic_und_x > def_center_x ? -1.f : 1.f) *
                                  0.001f;
    ...
    case fm_UVUxUyVxVy:
      ...
      case UVUxUyVxVyIC_DU:
        ...
        model_parameters[2] = prev_model_parameters[2] +
                              (ic_def_x - ic_und_x) / (ic_und_x - def_center_x);
        model_parameters[3] = prev_model_parameters[3] +
                              (ic_def_y - ic_und_y) *
                                  (ic_und_x > def_center_x ? -1.f : 1.f) *
                                  0.001f;
      ...
      case UVUxUyVxVyIC_DV:
        ...
        model_parameters[4] = prev_model_parameters[4] +
                              (ic_def_x - ic_und_x) *
                                  (ic_und_y > def_center_y ? -1.f : 1.f) *
                                  0.001f;
        model_parameters[5] = prev_model_parameters[5] +
                              (ic_def_y - ic_und_y) / (ic_und_y - def_center_y);
    ...
    if (adjusting_domain) // moving existing domain
    {
      ...
      case domain_annular:
        ...
        case annularIC_Center:
          ...
          center_x =
              prev_center_x + adjust_domain_right_x - adjust_domain_left_x;
          center_y =
              prev_center_y + adjust_domain_right_y - adjust_domain_left_y;
          break;
      ...
      case domain_blob:
        ...
        for (int i = 0; i < (int)xy_blob.size(); ++i) {
          xy_blob[i].first = prev_xy_blob[i].first + adjust_domain_right_x -
                             adjust_domain_left_x;
          xy_blob[i].second = prev_xy_blob[i].second + adjust_domain_right_y -
                              adjust_domain_left_y;
        }
      ...
      case domain_annular:
        ...
        inside_radius =
            makeAnnularRi(right_x, right_y, left_x, left_y, ri_by_ro);
...

      {
        ...
        for (int i = 1; i < (int)xy_blob.size(); ++i) {
          ...
          if (xy_blob[i].first > blob_x_max)
            blob_x_max = xy_blob[i].first;
          if (xy_blob[i].first < blob_x_min)
            blob_x_min = xy_blob[i].first;
          if (xy_blob[i].second > blob_y_max)
            blob_y_max = xy_blob[i].second;
          if (xy_blob[i].second < blob_y_min)
            blob_y_min = xy_blob[i].second;
        }
  ...
  // We are selecting a domain
    ...
    case domain_rectangular:
      ...
      printf("%15s %30s: center x = %6.2f  center_y = %6.2f  left_x = %6.2f  "
             "right_x = %6.2f  left_y = %6.2f  right_y = %6.2f  scale_x = "
             "%6.2f  scale_y = %6.2f\n",
             "und_imageLabel", "mouseRelease", center_x, center_y, left_x,
             right_x, left_y, right_y, clickScale_x, clickScale_y);
#endif

      // Calls enableRectangularCorrelate in MainApp
      emit valueChanged_rectangularSelected(center_x, center_y, right_x,
                                            right_y, left_x, left_y,
                                            clickScale_x, clickScale_y);
    ...
    case domain_annular:
      ...
      emit valueChanged_annularSelected(center_x, center_y, inside_radius,
                                        outside_radius, ri_by_ro, clickScale_x,
                                        clickScale_y);
...
void ImageLabel::GUIupdated() {
  ...
  case domain_rectangular:
    ...
    printf("%15s %30s: center x = %6.2f  center_y = %6.2f  left_x = %6.2f  "
           ...
           "scale_y = %6.2f\n",
           "und_imageLabel", "GUIupdated", center_x, center_y, left_x, right_x,
           left_y, right_y, clickScale_x, clickScale_y);
#endif

    // Calls mirrorSelectionRectangular on the def_imageLabel
    emit valueChanged_selectingRectangular(center_x, center_y, right_x, right_y,
                                           left_x, left_y, clickScale_x,
                                           clickScale_y);
  ...
  case domain_blob:
    ...
    // Calls mirrorSelectionBlob on the def_imageLabel
    emit valueChanged_selectingBlob(center_x, center_y, xy_blob, clickScale_x,
                                    clickScale_y);
...
void ImageLabel::applyModelRectangular() {
  if (mirroring) {
    ...
    case fm_UVQ:
      ...
      def_hi_left_x = left_x + model_parameters[0] * realScale_x +
                      (left_y - center_y) * model_parameters[2];
      def_lo_right_x = right_x + model_parameters[0] * realScale_x +
                       (right_y - center_y) * model_parameters[2];
      def_hi_left_y = left_y + model_parameters[1] * realScale_y +
                      (left_x - center_x) * model_parameters[2];
      def_lo_right_y = right_y + model_parameters[1] * realScale_y +
                       (right_x - center_x) * model_parameters[2];
    ...
    case fm_UVUxUyVxVy:
      ...
      def_hi_left_x = left_x + model_parameters[0] * realScale_x +
                      (left_x - center_x) * model_parameters[2] +
                      (left_y - center_y) * model_parameters[3];
      def_lo_right_x = right_x + model_parameters[0] * realScale_x +
                       (right_x - center_x) * model_parameters[2] +
                       (right_y - center_y) * model_parameters[3];
      def_hi_left_y = left_y + model_parameters[1] * realScale_y +
                      (left_x - center_x) * model_parameters[4] +
                      (left_y - center_y) * model_parameters[5];
      def_lo_right_y = right_y + model_parameters[1] * realScale_y +
                       (right_x - center_x) * model_parameters[4] +
                       (right_y - center_y) * model_parameters[5];
...
void ImageLabel::drawOutline_rectangular(QPainter &painter) {
  ...
  for (int i = 0; i <= h_subdivisions; i++) {
    painter.drawLine(def_hi_left_x + dx * i, def_hi_left_y + dvdx * i,
                     def_lo_left_x + dx * i, def_lo_left_y + dvdx * i);
    for (int j = 0; j <= v_subdivisions; j++)
      painter.drawLine(def_lo_left_x + dudy * j, def_lo_left_y + dy * j,
                       def_lo_right_x + dudy * j, def_lo_right_y + dy * j);
  }
...
void ImageLabel::drawOutline_annular(QPainter &painter) {
  ...
  for (int ri = 0; ri <= r_subdivisions; ++ri) {
    ...
    painter.drawEllipse(def_center_x - r / 2,
                        def_center_y - (r * realScale_y / realScale_x) / 2, r,
                        r * realScale_y / realScale_x);
    for (int ai = 1; ai < a_subdivisions; ++ai)
      painter.drawLine(def_center_x + def_inside_radius * cos(def_q + ai * da),
                       def_center_y +
                           def_inside_radius * sin(def_q + ai * da) *
                               realScale_y / realScale_x,
                       def_center_x + def_outside_radius * cos(def_q + ai * da),
                       def_center_y +
                           def_outside_radius * sin(def_q + ai * da) *
                               realScale_y / realScale_x);
  }
  painter.drawLine(
      def_center_x + def_inside_radius * cos(def_q),
      def_center_y + def_inside_radius * sin(def_q) * realScale_y / realScale_x,
      def_center_x + def_outside_radius * 1.1 * cos(def_q),
      def_center_y +
          def_outside_radius * 1.1 * sin(def_q) * realScale_y / realScale_x);
...
void ImageLabel::drawOutline_blob(QPainter &painter) {
  ...
  if (size) {
    for (int i = 1; i < size; ++i) {
      ...
      painter.drawLine(
          x0 + def_u + def_dudx * (x0 - center_x) + def_dudy * (y0 - center_y),
...
void ImageLabel::paint_inside_points(QPainter &painter) {
  QColor transparent_red(0xFF, 0x00, 0x00,
                         std::min(255, (int)(50.f * realScale_x * 2.f)));
  QColor transparent_green(0x00, 0xFF, 0x00,
                           std::min(255, (int)(50.f * realScale_x * 2.f)));
  QColor transparent_red_past(0xFF, 0x00, 0x00,
                              std::min(128, (int)(25.f * realScale_x * 2.f)));
  QColor transparent_green_past(0x00, 0xFF, 0x00,
                                std::min(128, (int)(25.f * realScale_x * 2.f)));
  ...
  if (n_points) {
    for (int p = 0; p < n_points; ++p) {
      ...
      if (size)
        for (int i = 1; i < size; ++i)
          painter.drawPoint(inside_points[p][i].first,
                            inside_points[p][i].second);
    }
  }
}
...
void ImageLabel::paint_contour_points(QPainter &painter) {
  ...
  QColor transparent_red(
      0xFF, 0x00, 0x00,
      170); // std::min(255, (int)(50.f * realScale_x * 2.f ) ) );
  QColor transparent_green(
      0x00, 0xFF, 0x00,
      170); // std::min(255, (int)(50.f * realScale_x * 2.f ) ) );
    ...
    for (int c = 0; c < n_contours; ++c) {
      ...
      if (size) {
        for (int i = 0; i < size - 1; ++i)
          painter.drawLine(
              contour_points[c][i].first, contour_points[c][i].second,
              contour_points[c][i + 1].first, contour_points[c][i + 1].second);

        painter.drawLine(contour_points[c][size - 1].first,
                         contour_points[c][size - 1].second,
                         contour_points[c][0].first,
                         contour_points[c][0].second);
      }
    }
  }
}

void ImageLabel::set_inside_points(v_points inside_points_vector_in,
                                   bool error_in) {
  error = error_in;
  ...
  for (unsigned int i = 0; i < inside_points_vector_in.size(); ++i) {
    temp.push_back(
        std::make_pair(inside_points_vector_in[i].first * realScale_x,
                       inside_points_vector_in[i].second * realScale_y));
  }
...
void ImageLabel::set_contour_points(v_points contour_points_vector_in,
                                    bool error_in) {
  error = error_in;
  ...
  for (unsigned int i = 0; i < contour_points_vector_in.size(); ++i) {
    temp.push_back(
        std::make_pair(contour_points_vector_in[i].first * realScale_x,
                       contour_points_vector_in[i].second * realScale_y));
  }
...
void ImageLabel::mirrorSelectionRectangular(float center_x_in,
                                            float center_y_in, float right_x_in,
                                            float right_y_in, float left_x_in,
                                            float left_y_in,
...
#if DEBUG_DOMAIN_SELECTION
//    printf("%15s %30s: center x = %6.2f  center_y = %6.2f  left_x = %6.2f
//    right_x = %6.2f  left_y = %6.2f  right_y = %6.2f  scale_x = %6.2f  scale_y
//    = %6.2f\n",
...
// Comes from emit valueChanged_selecting();
void ImageLabel::mirrorSelectionBlob(float center_x_in, float center_y_in,
