...
//  This functor is used to aggregate the deformation points resulting from one correlation step with
//      correlation parameters into the next set of undeformed points. Next undeformed points need to
//      be integer values, since the undeformed intensity values are queried from the undeformed image
...
struct add_pair
{
    ...
    {
        return std::make_pair ( (int) ( x.first + y.first + 0.5f ) , (int) ( x.second + y.second + 0.5f ) );
    }
...
void managerClass::set_color_mode( colorEnum color_mode_in, int number_of_colors_in )
{
...
void managerClass::set_update( updateEnum update_in )
{
    update = update_in;
}
...
{
    model = model_in;
    number_of_model_parameters = ModelClass::get_number_of_model_parameters( model );
}
...
void managerClass::set_max_iters( int max_iters_in )
{
...
void managerClass::set_error_handling_mode( errorHandlingModeEnum error_handling_mode_in )
{
    error_handling_mode = error_handling_mode_in;
}

void managerClass::set_undeformed_image( std::string und_fileName_in , int current_frame_order , referenceImageEnum referenceImage )
{  
    switch ( processor )
    {
        case processor_CPU:
        ...
        case processor_GPU:
            ...
            if ( current_frame_order == 0 )
            {
                // First undeformed pyramid is already made by the mainApp::updateGpuPyramids, which is called
                // whenever we 1)load new images or 2)change the pyramid parameters or 3)change the color mode
                // or 4)are done correlating or 5)change processor type. Via cuda_manager::resetImagePyramids
            }
            else if ( referenceImage == refImage_Previous )
            {
...
void managerClass::set_deformed_image( std::string def_fileName_in , int current_frame_order )
{
    switch ( processor )
    {
        case processor_CPU:
        ...
        case processor_GPU:
            ...
            if ( current_frame_order == 0 )
            {
                // First deformed pyramid is already made by the mainApp::updateGpuPyramids, which is called
                // whenever we 1) load new images or 2)change the pyramid parameters or 3)change the color mode
                // or 4)are done correlating or 5)change processor type. Via cuda_manager::resetImagePyramids
...
bool managerClass::perform_single_frame_correlation_rectangular( int hs , int vs , frame_results * results_in , int current_frame_order )
{
    ...
    // Float set of square dimensions, with float arithmetic for precise positioning of centers
    ...
    for ( int i = 0 ; i < hs ; ++i )
    {
        ...
        for ( int j = 0 ; j < vs ; ++j )
        {
            ...
            adjust_rectangular_domain(  center_x ,
                                        center_y ,
                                       &results_in[ iSector ] ,
                                        current_frame_order );   // want later to make parameters i and j, not center_x and center_y

            adjust_initial_guess     (  initial_guess ,
                                       &results_in[ iSector ] ,
                                        current_frame_order );

            // Definition of the undeformed xy points and number_of_points for this sector.
            std::chrono::system_clock::time_point start_point_selection = std::chrono::system_clock::now();

            if ( current_frame_order == 0 )
            {
                results_in[ iSector ].und_contour =
                        get_contour_points_rectangularDomain( center_x - xdim,
                                                              center_y - ydim,
                                                              center_x + xdim,
                                                              center_y + ydim  );
                switch ( processor )
                {
                    ...
                    results_in[ iSector ].und_inside_points =
                            get_inside_points_rectangularDomain(  center_x - xdim,
                                                                  center_y - ydim,
                                                                  center_x + xdim,
                                                                  center_y + ydim  );
                        break;
                    ...
                    case processor_GPU:
                        #if CUDA_ENABLED
                            cuda_manager->resetPolygon(  iSector ,  center_x - xdim,
                                                                    center_y - ydim,
                                                                    center_x + xdim,
                                                                    center_y + ydim  );
                            ...
                            {
                                results_in[ iSector ].und_inside_points = cuda_manager->getUndXY0ToCPU( iSector );
                            }
                        #endif
                        break;
            ...
            else // Logic to get the undeformed points on frames other than the first
            {
                switch ( deformationDescription )
                {
                    ...
                    case def_strict_Lagrangian:
                        results_in[ iSector ].und_inside_points = results_in[ iSector ].def_inside_points;
                    ...
                    case def_Lagrangian:
                        {
                            std::pair<float,float> center_offset = std::make_pair
                                    (
                                        results_in[ iSector ].und_center_x -
            ...
            std::chrono::system_clock::time_point duration_point_selection = std::chrono::system_clock::now();
            time_all_point_selection += (float)std::chrono::duration_cast<std::chrono::milliseconds>(duration_point_selection - start_point_selection).count() / 1000.f;
            #if DEBUG_TIME_POINT_SELECTION
                std::cout << "manager()     :point selection wall execution time(s): " << time_all_point_selection << '\n';
            #endif
            ...
            {
                case processor_CPU:
                    initial_guess = correlator->Newton_Raphson(	initial_guess,
                                                                (int) results_in[ iSector ].und_inside_points.size(),
                                                                center_x,
                                                                center_y,
                ...
                case processor_GPU:
                    ...
                    correlationResults = cuda_manager->correlate( iSector , initial_guess , results_in[ iSector ] );
            ...
            // Updating center and other correlation results from the correlator for the report
            ...
            switch ( processor )
            {
                ...
                case processor_GPU:
                    ...
                    if ( plot_inside_points )
                    {
                        results_in[ iSector ].def_inside_points = cuda_manager->getDefXY0ToCPU( iSector );
                    }
                    #endif
                    break;
            ...
            if ( plot_inside_points )
            {
                //connect(manager, SIGNAL( send_und_inside_points (v_points, bool, float, float) ), und_imageLabel, SLOT( set_inside_points (v_points, bool, float, float) ) );
                //connect(manager, SIGNAL( send_def_inside_points (v_points, bool, float, float) ), def_imageLabel, SLOT( set_inside_points (v_points, bool, float, float) ) );
            ...
            {
                results_in[ iSector ].def_contour = deformPoints(  results_in[ iSector ].und_contour,
                                                                   results_in[ iSector ].und_center_x,
                                                                   results_in[ iSector ].und_center_y );

                //connect(manager, SIGNAL( send_und_contour_points(v_points, bool) ), und_imageLabel, SLOT( set_contour_points(v_points, bool) ) );
                //connect(manager, SIGNAL( send_def_contour_points(v_points, bool) ), def_imageLabel, SLOT( set_contour_points(v_points, bool) ) );
                if ( current_frame_order == 0 || referenceImage == refImage_Previous )
                {
            ...
            if ( error )
            {
                ...
                if ( error_handling_mode == errorMode_stopAll || error_handling_mode == errorMode_stopFrame )
                    break;
            }
        ...
        if ( stop_flag ) break;
        if ( error && ( error_handling_mode == errorMode_stopAll || error_handling_mode == errorMode_stopFrame ) )
            break;
    }

    // Update the global correlation results, that averages through all sector's results, like center.
...
bool managerClass::perform_single_frame_correlation_annular( int rs , int as , frame_results * results_in , int current_frame_order )
{
    ...
    for ( int i = 0; i < rs; ++i )
    {
        for ( int j = 0; j < as; ++j )
        {
            ...
            // The undeformed domain is selected depending on the deformation description mode and
            // whether we are in the first frame. Note "adjust_annular_domain" defines r and a.
            adjust_annular_domain( i, j, r, a, ri, dr, da, cx, cy, as, results_in , current_frame_order );
            // Adjust_initial_guess customizes global_initial_guess for this sector, into initial_guess
            adjust_initial_guess ( initial_guess,
                                  &results_in[ iSector ],
                                   current_frame_order );
            ...
            if ( current_frame_order == 0 )
            {
                ...
                switch ( processor )
                {
                    ...
                    if ( plot_inside_points )
                    {
                        results_in[ i * as + j ].und_inside_points = cuda_manager->getUndXY0ToCPU( iSector );
                    }
                    #endif
                        break;
            ...
            {
                case processor_CPU:
                    correlator->Newton_Raphson( initial_guess,
                                                (int) results_in[ iSector ].und_inside_points.size(),
                                                (float*) results_in[ iSector ].und_inside_points.data() );
            ...
            if ( plot_inside_points )
            {
                //connect(manager, SIGNAL( send_und_inside_points (v_points, bool, float, float) ), und_imageLabel, SLOT( iside_points (v_points, bool, float, float) ) );
                //connect(manager, SIGNAL( send_def_inside_points (v_points, bool, float, float) ), def_imageLabel, SLOT( set_inside_points (v_points, bool, float, float) ) );
                if ( current_frame_order == 0 || referenceImage == refImage_Previous )
                {
                    emit send_und_inside_points( results_in[ iSector ].und_inside_points , error );
...
v_points managerClass::get_inside_points_annularDomain( float r , float dr , float a , float da ,
   ...
   #pragma omp parallel
    for ( float i = x0 ; i < x1 ; ++i )
    {
        ...
        for ( int j = y0 ; j < y1 ; ++j )
        {
            ...
            if ( r2 > ri2 && r2 < ro2 )
            {
                float cross1 = ( corner_11_x - i ) * ( corner_01_y - corner_11_y )  -  ( corner_11_y - j ) * ( corner_01_x - corner_11_x );
                float cross2 = ( corner_00_x - i ) * ( corner_10_y - corner_00_y )  -  ( corner_00_y - j ) * ( corner_10_x - corner_00_x );
    ...
    float total = (float)std::chrono::duration_cast<std::chrono::milliseconds>(duration - start).count() / 1000.f;
    std::cout << "manager()     :get_inside_points_annularDomain: wall execution time(s): " << total << '\n';
    #endif

    return inside_points_vector;
}

v_points managerClass::get_contour_points_annularDomain( float r , float dr , float a , float da ,
                                   float cx , float cy )
{
    v_points contour;
    ...
    // The contour of the angular sector approximates the circular arcs as polylines.
    // Every segment in the polyline is the smallest of 2 options. Either covering a min arc angle or
    // having a length specified by the global blob segment distance.
    float angular_step_out = std::min( da / ( (float) sqrt( min_blob_segment_squared ) / (r + dr) ) , min_angular_step_deg * PI / 180.f);
    int number_of_intermediate_angular_steps_out = (int) floor ( da / angular_step_out );

    float angular_step_in = std::min( da / ( (float) sqrt( min_blob_segment_squared ) / (r   ) ) , min_angular_step_deg * PI / 180.f);
    int number_of_intermediate_angular_steps_in = (int) floor ( da / angular_step_in );

    contour.reserve(number_of_intermediate_angular_steps_out + number_of_intermediate_angular_steps_in + 4 );
    ...
    {
        contour.push_back( std::make_pair( cx + ( r + dr ) * cos( a + i * angular_step_out ) , cy + ( r + dr ) * sin( a + i * angular_step_out ) ) );
    }
    ...
    {
        contour.push_back( std::make_pair( cx + ( r      ) * cos( a + i * angular_step_in  ) , cy + ( r      ) * sin( a + i * angular_step_in  ) ) );
    }

    return contour;
}

bool managerClass::perform_single_frame_correlation_blob( frame_results * results_in, int current_frame_order )
{
    int iSector = 0;
    ...
    // If the contour has 3 or more points (has a body) then find undeformed domain,
    // initial guess and the inside points
    {
        ...
        if ( current_frame_order == 0 )
        {
            // Code that computes the blob inside points is disabled. Now it is done with the polygon class
            //results_in[ 0 ].und_inside_points = get_inside_points_blobDomain( results_in[ 0 ].und_contour );
            ...
            switch ( processor )
            {
                ...
                if ( plot_inside_points)
                {
                    results_in[ 0 ].und_inside_points = cuda_manager->getUndXY0ToCPU( iSector );
                }
                #endif
                break;
    ...
    float duration_point_selection = (float)std::chrono::duration_cast<std::chrono::microseconds>( end_time - start_time ).count() / 1000000.f;
    time_all_point_selection += duration_point_selection;
    #if DEBUG_TIME_POINT_SELECTION
        std::cout<<"manager()     :point selection execution time(s): "<< duration_point_selection <<'\n';
    #endif
...
int managerClass::estimate_number_of_points( rectangularDomainStruct rectangularDomain_in )
{
...
int managerClass::estimate_number_of_points( annularDomainStruct annularDomain_in )
{
    ...
    // Allocate enough memory to compute correlation for any sector. Allow a growth factor of
    // 2 due to dilation. If more space is required, the correlator object takes care of reallocating
    // memory.
...
int managerClass::estimate_number_of_points( blobDomainStruct blobDomain_in )
{
    ...
    {
        if ( blobDomain_in.xy_contour[ p ].first  > x_max ) x_max = blobDomain_in.xy_contour[ p ].first;
        if ( blobDomain_in.xy_contour[ p ].first  < x_min ) x_min = blobDomain_in.xy_contour[ p ].first;
        if ( blobDomain_in.xy_contour[ p ].second > y_max ) y_max = blobDomain_in.xy_contour[ p ].second;
        if ( blobDomain_in.xy_contour[ p ].second < y_min ) y_min = blobDomain_in.xy_contour[ p ].second;
    }
...
void managerClass::perform_multiframe_correlation()
{
    ...
    std::chrono::system_clock::time_point chrono_start_multi_frame_correlation = std::chrono::system_clock::now();
    #endif
    ...
    {
        case processor_CPU:
            correlator  = new CorrelationClass( number_of_points,
                                                number_of_colors,
                                                interpolation,
                                                model,                                                
                                                number_of_threads,
        ...
        case processor_GPU:
            #if CUDA_ENABLED
            // GPU mode loads precision, max_iters, fitting model, interpolation model and number_of_color at mainApp::correlate.
    ...
    // Allocating memory for the resulting correlation parameters for one frame (a pair of images)
    ...
    for ( int i = 0 ; i < results_i ; ++i )
    {
        ...
        {
            results[ i * results_j + j ].resulting_parameters             = new float[ number_of_model_parameters ];
            results[ i * results_j + j ].initial_guess                    = new float[ number_of_model_parameters ];
            results[ i * results_j + j ].previous_resulting_parameters    = new float[ number_of_model_parameters ];
    ...
    for ( int current_frame_order = 0 ; current_frame_order < total_frame_pairs ; ++current_frame_order )
    {
        ...
        // Set images. They are read from file on the first frame. Subsequently, they are set from the next image
        ...
        //connect( manager , SIGNAL( display_images(QString, QString) ), this, SLOT( display_two_images( QString, QString ) ) );
        emit display_images( und_file_QString , def_file_QString );

        if ( plot_inside_points )
        {
            //    connect(manager, SIGNAL( clear_und_inside_points () ), und_imageLabel, SLOT( clear_inside_points () ) );
            //    connect(manager, SIGNAL( clear_def_inside_points () ), def_imageLabel, SLOT( clear_inside_points () ) );
            if ( current_frame_order == 0 || referenceImage == refImage_Previous )
            {
        ...
        if ( plot_contour_points )
        {
            //connect(manager, SIGNAL( clear_und_contour_points() ), und_imageLabel, SLOT( clear_contour_points() ) );
            //connect(manager, SIGNAL( clear_def_contour_points() ), def_imageLabel, SLOT( clear_contour_points() ) );
            if ( current_frame_order == 0 || referenceImage == refImage_Previous )
            {
        ...
        {
            std::string next_file_string = fileNames->at( current_frame_order + 2 ).toStdString();
            nextImageLoaderThread = std::async( std::launch::async , &managerClass::set_next_image , this , next_file_string );
        }

        switch ( domain_type )
        {
            case domain_rectangular:
                perform_single_frame_correlation_rectangular( results_i , results_j , results , current_frame_order );

                break;

            case domain_annular:
                perform_single_frame_correlation_annular( results_i , results_j , results , current_frame_order );
                break;
        ...
        #if DEBUG_TIME_FRAME_CORRELATION
        float duration_frame_correlation = ( std::clock() - start_frame_correlation ) / (float) CLOCKS_PER_SEC;
        std::cout<<"manager()     :Frame ";
        std::cout << current_frame_order << " correlation execution time(s): "<< duration_frame_correlation <<'\n';
        #endif

        // Writes the text display in mainApp with parameters and angle per
        // connect(manager, SIGNAL( display_results( float , float* ) ), this , SLOT( updateResultDisplay( float , float* ) ) );
        ...
        if ( stop_flag || ( error && error_handling_mode == errorMode_stopAll ) ) break;
    ...
    //connect(manager, SIGNAL( correlation_is_done(bool) ), this, SLOT( correlation_done(bool) ) );
    emit correlation_is_done( error );

    #if DEBUG_TIME_MULTI_FRAME_CORRELATION
    std::chrono::system_clock::time_point chrono_duration_multi_frame_correlation = std::chrono::system_clock::now();
    std::cout<<"manager()     :Multi-frame correlation wall execution time(s): " <<
          (float)std::chrono::duration_cast<std::chrono::milliseconds>(chrono_duration_multi_frame_correlation - chrono_start_multi_frame_correlation ).count() / 1000.f <<'\n';
    fflush( stdout );
    #endif

    #if DEBUG_TIME_ALL_POINT_SELECTION
        std::cout<<"manager()     :ALL point selection execution time(s): "<< time_all_point_selection <<'\n';
    #endif
}

void managerClass::set_deformation_description( deformationDescriptionEnum deformationDescription_in )
{
...
v_points managerClass::get_inside_points_blobDomain( v_points xy_contour_in )
{
    ...
    {
        if ( xy_contour_in[i].first  > f_max_x ) f_max_x = xy_contour_in[i].first;
        if ( xy_contour_in[i].first  < f_min_x ) f_min_x = xy_contour_in[i].first;
        if ( xy_contour_in[i].second > f_max_y ) f_max_y = xy_contour_in[i].second;
        if ( xy_contour_in[i].second < f_min_y ) f_min_y = xy_contour_in[i].second;
    }
    ...
    if ( convex )
    {
        inside_points =  get_inside_points_blobDomain_convex    ( xy_contour_in , max_x , min_x , max_y , min_y );
    }
    else
    {
        inside_points =  get_inside_points_blobDomain_non_convex( xy_contour_in , max_x , min_x , max_y , min_y );
    }
    ...
    for (unsigned int i = 0; i < xy_contour_in.size(); ++i )
            std::cout << "Contour Point "  << i << " = " << xy_contour_in[i].first << " "  << xy_contour_in[i].second << std::endl;
    for (unsigned int i = 0; i < inside_points.size(); ++i )
            std::cout << "Body Point "  << i << " = " << inside_points[i].first << " "  << inside_points[i].second << std::endl;
#endif
    return inside_points;
}

v_points managerClass::get_inside_points_rectangularDomain( int x0 , int y0 , int x1 , int y1 )
{
    #if DEBUG_MANAGER_INSIDE_POINTS
...
v_points managerClass::get_contour_points_rectangularDomain( int x0 , int y0 , int x1 , int y1 )
{
    v_points contour;
    contour.reserve( 4 );
...
v_points managerClass::get_inside_points_blobDomain_non_convex( v_points xy_contour_in,
                                                                int max_x ,  int min_x ,
                                                                int max_y ,  int min_y )
{
    #if DEBUG_MANAGER_INSIDE_POINTS
...
#pragma omp parallel
    {
        ...
        for ( int iy = min_y ; iy <= max_y ; ++iy )
        {
            ...
            {
                if ( check_inside_polygon ( xy_contour_private , ix , iy , contourLineEquations_private ) )
                {
...
v_points managerClass::get_inside_points_blobDomain_convex( v_points xy_contour_in,
                                                            int max_x ,  int min_x,
                                                            int max_y ,  int min_y )
{
    #if DEBUG_MANAGER_INSIDE_POINTS
    ...
    if ( min_y == max_y )
    {
        ...
        else
        {
            ...
            v_points quadrant1 = get_inside_points_blobDomain_convex( xy_contour_in, x1, x0    , max_y, min_y );
            v_points quadrant2 = get_inside_points_blobDomain_convex( xy_contour_in, x2, x1 + 1, max_y, min_y );
    ...
    if ( min_x == max_x )
    {
        ...
        else
        {
            ...
            v_points quadrant1 = get_inside_points_blobDomain_convex( xy_contour_in , max_x , min_x , y1 , y0     );
            v_points quadrant3 = get_inside_points_blobDomain_convex( xy_contour_in , max_x , min_x , y2 , y1 + 1 );
    ...
    if ( check_minx_miny && check_minx_maxy && check_maxx_miny && check_maxx_maxy )
    {
    ...
    else
    {
        ...
        v_points quadrant1 = get_inside_points_blobDomain_convex( xy_contour_in , x1 , x0     , y1 , y0     );
        v_points quadrant2 = get_inside_points_blobDomain_convex( xy_contour_in , x2 , x1 + 1 , y1 , y0     );
        v_points quadrant3 = get_inside_points_blobDomain_convex( xy_contour_in , x1 , x0     , y2 , y1 + 1 );
        v_points quadrant4 = get_inside_points_blobDomain_convex( xy_contour_in , x2 , x1 + 1 , y2 , y1 + 1 );
...
bool managerClass::check_polygon_convex( v_points xy_contour_in )
{
    ...
    if ( zcrossproduct * ( dx1 * dy2 - dy1 * dx2 ) < 0.f ) return false;
...
{
    polyEquations contourLineEquations( xy_contour_in.size() , std::vector< float >( 3 , 0 ) );
    ...
    contourLineEquations[ xy_contour_in.size() - 1 ][ 2 ] = ( v1x2 * v1y1 ) - ( v1x1 * v1y2 );

    return contourLineEquations;
}

bool managerClass::check_inside_polygon( const v_points &xy_contour_in , int ix , int iy )
{
    int n = (int) xy_contour_in.size();
    ...
    if ( check_segment_intersection( v1x1 , v1y1 , v1x2 , v1y2 ,  v2x1 , v2y1 , v2x2 , v2y2 ) == intersection_yes )
    {
    ...
    if ( number_intersections % 2 != 0 ) return true;
                                         ...
                                         const polyEquations &contourLineEquations )
{
    ...
    for ( int i = 0 ; i < n ; ++i )
    {
        ...
        if ( check_segment_intersection( (float)ix , (float)iy ,
                                         v2y1 , v2y2 ,
                                         contourLineEquations[ i ] ) == intersection_yes )
        {
            number_intersections++;
        }
    }
...
intersectionEnum managerClass::check_segment_intersection(  float v1x1 , float v1y1 , float v1x2 , float v1y2,
...
intersectionEnum managerClass::check_segment_intersection( float v1x2, float v1y2,
                                                           float v2y1, float v2y2,
...
void managerClass::adjust_rectangular_domain( int &center_x , int &center_y , frame_results *previous_results, int current_frame_order )
{
    ...
    // After the first Frame, we use the "pevious_results" to define the sector domains.
    else
    {
        switch( deformationDescription )
        {
            ...
            case def_Eulerian:
            {
                ...
                previous_results->und_global_center_x      = previous_results->und_global_center_x;
                previous_results->und_global_center_y      = previous_results->und_global_center_y;
            ...
            // In Lagrangian mode, the correlation follows material points. So the correlation of square
            // x is started where it ended up in the previous frame.
            case def_strict_Lagrangian:
            case def_Lagrangian:
            {

                previous_results->und_global_center_x = previous_results->def_global_center_x;
                previous_results->und_global_center_y = previous_results->def_global_center_y;
...
void managerClass::adjust_annular_domain(int i, int j, float &r, float &a, float &ri, float dr, float da, float &center_x, float &center_y, int as, frame_results *all_previous_results, int current_frame_order)
{
    // This method modifies the annular domain in a cohesive way. New center for the annulus is computed.
    if (  current_frame_order == 0  )
    {
        all_previous_results[ i * as + j ].und_global_center_x    = annularDomain.x_center;
        all_previous_results[ i * as + j ].und_global_center_y    = annularDomain.y_center;
        ...
        all_previous_results[ i * as + j ].und_global_e           = 0.f;  //first frames assumes zero a and e.
        all_previous_results[ i * as + j ].und_global_angle       = 0.f;

        // temporary value to compute sector initial_guess. After correlation, value gets updated with values from
        // correlator object
        if ( as > 1 ) // 2 or more angular subdivisions
        {
            float center_angle = 0  + j * da
                    + da / 2.f;
            float center_r     = ri + i * dr
                    + dr / 2.f;

            all_previous_results[ i * as + j ].und_center_x         = all_previous_results[ i * as + j ].und_global_center_x +
                                                                        center_r * (float) cos( (double) center_angle );
            all_previous_results[ i * as + j ].und_center_y         = all_previous_results[ i * as + j ].und_global_center_y +
                                                                        center_r * (float) sin( (double) center_angle );
        }
        //  If we only have one angular subdivision, the local center is the same as the global center
        else
        {
            all_previous_results[ i * as + j ].und_center_x         = all_previous_results[ i * as + j ].und_global_center_x;
            all_previous_results[ i * as + j ].und_center_y         = all_previous_results[ i * as + j ].und_global_center_y;
        ...
        all_previous_results[ i * as + j ].past_und_center_x        = all_previous_results[ i * as + j ].und_center_x;
        all_previous_results[ i * as + j ].past_und_center_y        = all_previous_results[ i * as + j ].und_center_y;

        all_previous_results[ i * as + j ].und_e                    = 0.f;  //first frames assumes zero a and e.
    ...
    else
    {
        switch( deformationDescription )
        {
            ...
            case def_Eulerian:
            {
                ...
                all_previous_results[ i * as + j ].und_global_center_x      = all_previous_results[ i * as + j ].und_global_center_x;
                all_previous_results[ i * as + j ].und_global_center_y      = all_previous_results[ i * as + j ].und_global_center_y;

                all_previous_results[ i * as + j ].und_global_ro            = all_previous_results[ i * as + j ].und_global_ro;
                all_previous_results[ i * as + j ].und_global_ri            = all_previous_results[ i * as + j ].und_global_ri;
                all_previous_results[ i * as + j ].und_global_e             = all_previous_results[ i * as + j ].und_global_e;
                all_previous_results[ i * as + j ].und_global_angle         = all_previous_results[ i * as + j ].und_global_angle;

                // temporary value to compute sector initial_guess. After correlation, value gets updated with values from
                // correlator object
                all_previous_results[ i * as + j ].und_center_x             = all_previous_results[ i * as + j ].und_center_x;
                all_previous_results[ i * as + j ].und_center_y             = all_previous_results[ i * as + j ].und_center_y;

                all_previous_results[ i * as + j ].und_e                    = all_previous_results[ i * as + j ].und_e;
                all_previous_results[ i * as + j ].und_angle                = all_previous_results[ i * as + j ].und_angle;

                break;

            }// Eulerian description brackets

            // In Lagrangian mode, the correlation follows material points. So the correlation of the blob
            // starts where it ended up in the previous frame.
            case def_strict_Lagrangian:
            case def_Lagrangian:
            {
                all_previous_results[ i * as + j ].und_global_center_x    = all_previous_results[ i * as + j ].def_global_center_x;
                all_previous_results[ i * as + j ].und_global_center_y    = all_previous_results[ i * as + j ].def_global_center_y;

                all_previous_results[ i * as + j ].und_global_ro          = all_previous_results[ i * as + j ].def_global_ro;
                all_previous_results[ i * as + j ].und_global_ri          = all_previous_results[ i * as + j ].def_global_ri;
                all_previous_results[ i * as + j ].und_global_e           = all_previous_results[ i * as + j ].def_global_e;
                all_previous_results[ i * as + j ].und_global_angle       = all_previous_results[ i * as + j ].def_global_angle;

                // temporaty value to compute sector initial_guess. After correlation, value gets updated with values from
                // correlator object
                all_previous_results[ i * as + j ].past_und_center_x      = all_previous_results[ i * as + j ].und_center_x;
                all_previous_results[ i * as + j ].past_und_center_y      = all_previous_results[ i * as + j ].und_center_y;

                all_previous_results[ i * as + j ].und_center_x           = all_previous_results[ i * as + j ].def_center_x;
                all_previous_results[ i * as + j ].und_center_y           = all_previous_results[ i * as + j ].def_center_y;

                all_previous_results[ i * as + j ].und_e                  = all_previous_results[ i * as + j ].def_e;
                all_previous_results[ i * as + j ].und_angle              = all_previous_results[ i * as + j ].def_angle;
...
void managerClass::adjust_blob_domain( frame_results * previous_results, int current_frame_order )
{
    // The "previous results" struct have not been set yet.
    if (  current_frame_order == 0  )
    {
    ...
    // After the first Frame, we use the "pevious_results" to define the blob domains.
    else
    {
        switch( deformationDescription)
        {
...
void managerClass::update_results( frame_results *results_in , CorrelationResult *correlationResult )
{
    ...
    memcpy( results_in->resulting_parameters,
            temp_model_parameters ,
            number_of_model_parameters );

    memcpy( initial_guess,
            temp_model_parameters ,
            number_of_model_parameters );
    ...
    case fm_UVQ:
        ...
        results_in->def_angle = results_in->resulting_parameters[ 2 ] +
    ...
    case fm_UVUxUyVxVy:
        ...
        results_in->def_angle = best_rotation_UVUxUyVxVy( results_in->resulting_parameters )
                                + results_in->und_angle;
        results_in->def_e     = 0.f;
        break;
    ...
    // Short names to variables that are required for the computation of the sectors centers.
    ...
    results_in->def_center_x        = distortX( x, y, x, y, ro, results_in->resulting_parameters );
    results_in->def_center_y        = distortY( x, y, x, y, ro, results_in->resulting_parameters );
    ...
    {
        float def_x = distortX( results_in->und_contour[ i ].first, results_in->und_contour[ i ].second, cx, cy, ro, results_in->resulting_parameters );
        float def_y = distortY( results_in->und_contour[ i ].first, results_in->und_contour[ i ].second, cx, cy, ro, results_in->resulting_parameters );
        results_in->def_contour.push_back( std::make_pair( def_x , def_y ) );
    }
}

void managerClass::addFrameToReport(int frame_number_in, int results_i_in, int results_j_in, frame_results * results_in)
{
    for ( int i = 0; i < results_i_in; ++i )
        for ( int j = 0; j < results_j_in; ++j )
        {
            report << frame_number_in << ","
                   << und_file_string << ","
                   << def_file_string << ","
...
v_points managerClass::deformPoints( v_points contour , float cx , float cy , float ro )
{
    ...
    {
        float x = def_x ( contour[ i ].first , contour[ i ].second , cx , cy , ro , model_parameters );
        float y = def_y ( contour[ i ].first , contour[ i ].second , cx , cy , ro , model_parameters );
                                          ...
                                          int current_frame_order )
{
    ...
    if ( current_frame_order == 0 )
    {
        switch ( model )
        {
            ...
            case fm_UVQ:
            {
                // Sectors on the UVQ models have the global initial guess plus the strain contribution.
                copy_model_parameters ( global_initial_guess , previous_results->initial_guess , number_of_model_parameters );

                float dx = previous_results->und_center_x - previous_results->und_global_center_x;
                float dy = previous_results->und_center_y - previous_results->und_global_center_y;
            ...
            case fm_UVUxUyVxVy:
            {
                // Sectors on the UVUxUyVxVy models have the global initial guess plus the strain contribution.
                copy_model_parameters ( global_initial_guess, previous_results->initial_guess, number_of_model_parameters );
    ...
    // else case applies to frames other than the first. The relsuts from the previous frame are a good guess for
    // the next frame.
    else
    {
        // Eulerian descriptions with a first image reference progresively increment the resulting parameters.
        // Under the assumption of constant rate of deformetion, we can bias the initial guess to account for this
        if ( deformationDescription == def_Eulerian && referenceImage == refImage_First)
        {

            for ( int i = 0; i < number_of_model_parameters; ++i )
            {
                previous_results->initial_guess[ i ] = previous_results->resulting_parameters[ i ] +
                        ( previous_results->resulting_parameters[ i ] - previous_results->previous_resulting_parameters[ i ] );
            }
    ...
    // For all types of frames, first and other, copy the computed initial guess (stored in the results) into the
    // active variable "initial_guess" read by the correlator.
    copy_model_parameters( previous_results->initial_guess, initial_guess, number_of_model_parameters );
}

void managerClass::update_global_results( int hrs, int vas, frame_results * all_results_in )
{
    ...
    for ( int i = 0 ; i < hrs ; ++i )
    {
        for ( int j = 0 ; j < vas ; ++j )
        {
            ...
            all_results_in[ i * vas + j ].def_global_ro      = all_results_in[ i * vas + j ].und_global_ro;
...
void managerClass::set_pyramid( int pyramid_start_in , int pyramid_step_in , int pyramid_stop_in )
{
...
errorEnum managerClass::get_errorType()
{
    return errorType;
}
