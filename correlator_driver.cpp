...
std::string errorStrings[4] = { "error_none" ,
...
int main()
{
	...
	int number_of_fitting_parameters = ModelClass::get_number_of_model_parameters(fittingModel); //get #parameters class method
	...
	#if DEBUG_MEMORY 
	std::cout << "allocating model_parameters  in driver= " << model_parameters << std::endl;
	#endif
	...
	//std::string filename_und = "../../../../data/checkerboard_blurred_bw_100x100.png";
	...
	//std::string filename_def = "../../../../data/checkerboard_blurred_bw_100x100_1x_5deg.png";
	...
	// //define the correlation problem - xy points in the undeformed image to correlate
	// int x_start = 5;
	// int x_end = 15; 
	...
	// //Dynamically allocate the interpolation parameters array and xy-position (und) for all threads to share
	// float * xy_positions = new float[ number_of_points * 2];
	// #if DEBUG_MEMORY  
	// std::cout << "allocating xy_positions in driver= " << xy_positions << std::endl;
	// #endif
	...
	// new_pars = correlator.Newton_Raphson(model_parameters, number_of_points, xy_positions);
	...
	std::cout << std::endl << "status = " << correlator.get_error_status() <<
	std::endl << "error code  = " <<
	 errorStrings[ correlator.get_error_code() ] <<
	 std::endl << "resulting model parameters = ";
	for(int p = 0 ; p < number_of_fitting_parameters ; ++p )
		std::cout << new_pars [ p ] << " ";
	std::cout << std::endl;

	#if DEBUG_MEMORY  
		std::cout << "deleting model_parameters in driver = " << model_parameters << std::endl; 
	#endif
