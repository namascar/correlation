...
void cudaImage::makePyramid( const cv::Mat &image_in )
{
    ...
    for ( int ilevel = 1 ; ilevel < stop + 1 ; ++ilevel )
    {
        ...
        dim3 blocksPerGrid (
                ( dstCols * number_of_colors + threadsPerBlock.x - 1 ) / threadsPerBlock.x ,
...
void cudaImage::allocateImages( const cv::Mat &image_in )
{
    ...
    for ( int ilevel = 0 ; ilevel < stop + 1 ; ++ilevel )
    {
        ...
        err = cudaMallocArray ( &d_images_cuArray[ ilevel ],
                                &channelDesc,
                                 cols * number_of_colors , rows );

        if ( err != cudaSuccess )
        {
            printf( "Failed to allocate memory for d_images_cuArray at level %d(error code %s)!\n", ilevel , cudaGetErrorString( err ) );
            exit( EXIT_FAILURE );
        }

        #if DEBUG_CUDA_PYRAMID
            printf("cuda_pyramid: allocateImages: completed cuArray allocation on level %d pyramid type %d\n", ilevel , imageType );
        #endif
    }
}
...
void cudaImage::uploadImage0( const cv::Mat &image_in )
{
    ...
    err = cudaMemcpyToArrayAsync(   d_images_cuArray[ 0 ],
                                    0,
                                    0,
                                    image_in.data,
                                    rowsLevel0 * colsLevel0 * number_of_colors * sizeof( unsigned char ),
                                    cudaMemcpyHostToDevice , stream );

    if ( err != cudaSuccess )
    {
        printf( "Failed to copy image_in to d_images_cuArray[ 0 ] (error code: %s)!\n", cudaGetErrorString( err ) );
        exit( EXIT_FAILURE );
    }
}
...
void cudaImage::createAndBindTextures()
{
    ...
    for ( int ilevel = 0 ; ilevel < d_images_cuArray.size() ; ++ilevel )
    {
        ...
        err = cudaCreateTextureObject( &textures[ ilevel ] , &resDesc , &texDesc , nullptr );
        if ( err != cudaSuccess )
        {
        ...
        #if DEBUG_CUDA_PYRAMID
        printf( "cudaImage::createAndBindTextures: Binding texture of level %d pyramid of type %d\n" ,
                ilevel , imageType );
        #endif
    }
}
...
void cudaImage::createAndBindSurfaces()
{
    ...
    //  Create new surface objects on GPU0 - Surfaces are used to write to cudaArray,
    ...
    for ( int ilevel = 1 ; ilevel < d_images_cuArray.size() ; ++ilevel )
    {
        ...
        #if DEBUG_CUDA_PYRAMID
        printf( "cudaImage::createAndBindSurfaces: Binding surface of level %d pyramid of type %d\n" ,
                ilevel , imageType );
        #endif
    }
}
...
void cudaImage::testPyramid()
{
    ...
    for ( int ilevel = 0 ; ilevel < d_images_cuArray.size() ; ++ilevel )
    {
        ...
        err = cudaMemcpyFromArray(  images[ ilevel ].data,
                                    d_images_cuArray[ ilevel ],
                                    0,
                                    0,
                                    rows * cols * number_of_colors * sizeof( unsigned char ),
...
void cudaPyramid::updateParameters( int iSector,
                                    int numberOfModelParameters,
                                    parameterTypeEnum parSrc,
                                    parameterTypeEnum parDst,
                                    cudaStream_t stream )
{
    polygons[ iSector ]->updateParameters( numberOfModelParameters , parSrc , parDst , stream );
}
...
void cudaPyramid::initializeParametersLevel0( int iSector , float *initialGuess_ )
{
#if DEBUG_CUDA_PYRAMID
...
void cudaPyramid::transferParameters( int iSector , parameterTypeEnum parSrc , parameterTypeEnum parDst )
{
#if DEBUG_CUDA_PYRAMID
...
//    undFuture = std::async( std::launch::async , &cudaPyramid::getNewPyramid , this , undCvImage , imageType_und , undStream );
//    defFuture = std::async( std::launch::async , &cudaPyramid::getNewPyramid , this , defCvImage , imageType_def , defStream );
//    if ( !nxtCvImage.empty() )
//    {
//        nxtFuture = std::async( std::launch::async , &cudaPyramid::getNewPyramid , this , nxtCvImage , imageType_nxt , nxtStream );
//    }
...
void cudaImage::setImageType( ImageType imageType_ )
{
    imageType = imageType_;
}
...
{  
    polygons[ iSector ] = new cudaPolygonRectangular( iSector , x0 , y0 , x1 , y1 ,
                                          start , step , stop ,
                                          fittingModel_ );
}

void cudaPyramid::resetPolygon( int iSector , float r , float dr , float a , float da ,
...
void cudaPyramid::resetPolygon( v_points blobContour,
                                fittingModelEnum fittingModel_ )
{
    polygons[ 0 ] = new cudaPolygonBlob( blobContour ,
                                   start , step , stop ,
                                   fittingModel_ );
}
void cudaPyramid::updatePolygon( int iSector , deformationDescriptionEnum deformationDescription )
{
    polygons[ iSector ]->updatePolygon( deformationDescription );
}

int cudaPyramid::getPyramidStart()
{
    return start;
}

int cudaPyramid::getPyramidStep()
{
    return step;
}

int cudaPyramid::getPyramidStop()
{
    return stop;
}
