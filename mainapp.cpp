    ...
    ui( new Ui::MainApp )
{
    ...
    // Declare the inside points before connect declarations. Needed for queued signal-slot
    ...
    // Create a manager object to organize handling of multiples images and multiples
    ...
    deformation_descriptionList << "Strict Lagrangian" << "Lagrangian" << "Eulerian";
    deformation_description_iv = def_Eulerian;
    subdivisions->deformation_descriptionBox->addItems( deformation_descriptionList);
    subdivisions->deformation_descriptionBox->setCurrentIndex(deformation_description_iv);

    error_handlingModeList << "Stop all frames" << "Stop current frame" << "Ignore";
    ...
    reference_imageList << "First Image" << "Previous Image";
    ...
    // allows disableing the First image reference model in Lagrangian deformation descriptions
    comboBox_reference_image = qobject_cast<QStandardItemModel*>( subdivisions->reference_imageBox->model() );
    ...
    else item_first_ref_image->setEnabled( false );
    ...
    icList << "Null" << "Automatic" << "User Picked";
    ...
    interpolation_modelList << "Nearest" << "Bilinear" << "Bicubic";
    ...
    domainList << "Rectangular" << "Annular" << "Blob";
    ...
    // Fitting model
    fit_modelList << "U" << "U, V" << "U, V, Q" << "U, V, Ux, Uy, Vx, Vy";
    ...
    // Using the initial_guess_iv = Null and model_iv, puts togueter an initial guess and shares it
    ...
    //  Initial default directory name, moving forward code remembers last directory.
    last_directory_name = "/home/namascar/Documents/data/corr_img/stick_slip2/jpg";
...
bool MainApp::display_two_images( QString und_file_QString_in , QString def_file_QString_in )
{
...
bool MainApp::display_undeformed_image( QString und_file_QString_in )
{
    ...
    else
    {
        ...
        QString first_file_name = first_file_path.remove
                (0, first_file_path.lastIndexOf( "/" ) + 1 );
...
bool MainApp::display_deformed_image( QString def_file_QString_in )
{
    ...
    {
        if ( def_image.isGrayscale() ) number_of_colors = 1;
        ...
        QString second_file_name = second_file_path.remove
                (0, second_file_path.lastIndexOf( "/" ) + 1 );
...
{
#if AUTO_PILOT
//    fileNames << "/home/namascar/Documents/data/corr_img/translation1X2Y/frame-0.png"
//              << "/home/namascar/Documents/data/corr_img/translation1X2Y/frame-1.png";
        fileNames << "/home/namascar/Documents/data/corr_img/Eyes/CLAHE_1024x1024/Dock_clahe_1024x1024.png"
                  << "/home/namascar/Documents/data/corr_img/Eyes/CLAHE_1024x1024/Dock_clahe_1024x1024_1deg.png"
                  << "/home/namascar/Documents/data/corr_img/Eyes/CLAHE_1024x1024/Dock_clahe_1024x1024_2deg.png"
                  << "/home/namascar/Documents/data/corr_img/Eyes/CLAHE_1024x1024/Dock_clahe_1024x1024_3deg.png"
                  << "/home/namascar/Documents/data/corr_img/Eyes/CLAHE_1024x1024/Dock_clahe_1024x1024_4deg.png"
                  << "/home/namascar/Documents/data/corr_img/Eyes/CLAHE_1024x1024/Dock_clahe_1024x1024_5deg.png"
                  << "/home/namascar/Documents/data/corr_img/Eyes/CLAHE_1024x1024/Dock_clahe_1024x1024_6deg.png"
                  << "/home/namascar/Documents/data/corr_img/Eyes/CLAHE_1024x1024/Dock_clahe_1024x1024_7deg.png"
                  << "/home/namascar/Documents/data/corr_img/Eyes/CLAHE_1024x1024/Dock_clahe_1024x1024_8deg.png"
                  << "/home/namascar/Documents/data/corr_img/Eyes/CLAHE_1024x1024/Dock_clahe_1024x1024_9deg.png"
                  << "/home/namascar/Documents/data/corr_img/Eyes/CLAHE_1024x1024/Dock_clahe_1024x1024_10deg.png"
                    ;
//        fileNames << "/home/namascar/Documents/data/corr_img/Eyes/CLAHE_1024x1024/Dock_clahe_1024x1024.png"
//                  << "/home/namascar/Documents/data/corr_img/Eyes/CLAHE_1024x1024/Dock_clahe_1024x1024_1deg.png";
#else
    fileNames = QFileDialog::getOpenFileNames( this,
                  tr("Open Files"), last_directory_name );
#endif
...
{  
    ...
    connect(ui->action_Save_Analysis, SIGNAL(triggered()), this, SLOT(saveAnalysisReport()));
    ...
    connect(ui->action_Normal_Size, SIGNAL(triggered()), this, SLOT(normalSize()));
    ...
    connect(ui->actionParame_ters, SIGNAL( triggered() ), this, SLOT( open_subdivisions() ));
    connect(subdivisions->okButton, SIGNAL( released() ), this, SLOT( close_subdivisions() ));
    connect(subdivisions->deformation_descriptionBox, SIGNAL( currentIndexChanged(int) ), this, SLOT( change_deformation_description(int) ));
    connect(subdivisions->processorBox , SIGNAL( currentIndexChanged(int) ), this, SLOT( change_processor(int) ));
    connect(subdivisions->number_of_GPUs_threads_Box , SIGNAL( valueChanged( int ) ), this, SLOT( change_devices_used( int ) ));
    connect(subdivisions->preload_images_Box , SIGNAL( currentIndexChanged(int) ), this, SLOT( change_preload_images( int ) ));

    ui->pushButton_correlate->setEnabled(false);
    connect(ui->pushButton_correlate, SIGNAL( released() ), this, SLOT( correlate() ) );
    connect(this, SIGNAL( start_correlation() ), manager, SLOT( perform_multiframe_correlation() ) );

    //connection with the und_imageLabel and def_imageLabel that select and mirror domains
    connect(this          , SIGNAL(valueChanged_scaleSelection () ),
            und_imageLabel, SLOT  ( scaleSquare() ) );
    connect(this, SIGNAL(valueChanged_scaleSelection () ),
            def_imageLabel, SLOT( scaleSquare() ) );
    connect(this, SIGNAL( valueChanged_domain() ),
            und_imageLabel, SLOT(GUIupdated() ) );
    connect(und_imageLabel, SIGNAL(valueChanged_rectangularSelected( float, float, float, float, float, float, float, float) ),            
            this, SLOT( enableRectangularCorrelate( float, float, float, float, float, float, float, float) ) );
    connect(und_imageLabel, SIGNAL(valueChanged_annularSelected    ( float, float, float, float, float, float, float) ),            
            this, SLOT( enableAnnularCorrelate( float, float, float, float, float, float, float) ) );
    connect(und_imageLabel, SIGNAL(valueChanged_blobSelected       ( v_points, float, float ) ),
                                    this, SLOT( enableBlobCorrelate( v_points, float, float ) ) );


    connect(und_imageLabel, SIGNAL(valueChanged_selectingRectangular(float, float, float, float, float, float, float, float) ), def_imageLabel,
                                    SLOT( mirrorSelectionRectangular(float, float, float, float, float, float, float, float) ) );
    connect(und_imageLabel, SIGNAL(valueChanged_selectingAnnular(float, float, float, float, float, float, float) ), def_imageLabel,
                                    SLOT( mirrorSelectionAnnular(float, float, float, float, float, float, float) ) );
    connect(und_imageLabel, SIGNAL(valueChanged_selectingBlob(float, float, v_points, float , float) ), def_imageLabel,
                                    SLOT( mirrorSelectionBlob(float, float, v_points, float , float) ) );

    connect(und_imageLabel, SIGNAL(valueChanged_selectingRectangular(float, float, float, float, float, float, float, float) ), this,
                                    SLOT( refresh_images() ) );
    connect(und_imageLabel, SIGNAL(valueChanged_selectingAnnular(float, float, float, float, float, float, float) ), this,
                                    SLOT( refresh_images() ) );
    connect(und_imageLabel, SIGNAL(valueChanged_selectingBlob(float, float, v_points, float , float) ), this,
                                    SLOT( refresh_images() ) );
    ...
    connect(def_imageLabel, SIGNAL( new_initial_conditions() ), this, SLOT( updateResultDisplay() ) );
    connect(und_imageLabel, SIGNAL(update_mirror()), def_imageLabel, SLOT( update() ) );

    // Connection of the GUI controls
    connect(ui->comboBox_correlation_model, SIGNAL( currentIndexChanged(int) ),this, SLOT( change_correlation_model(int)) );
    connect(ui->comboBox_color, SIGNAL( currentIndexChanged(int) ),this, SLOT( change_color(int)) );
    connect(ui->comboBox_Initial_Guess, SIGNAL( currentIndexChanged(int) ),this, SLOT( change_Initial_Guess(int)) );
    connect(ui->comboBox_interpolation_model, SIGNAL( currentIndexChanged(int) ),this, SLOT( change_interpolation_model(int)) );
    connect(ui->comboBox_update, SIGNAL( currentIndexChanged(int) ),this, SLOT( change_update(int)) );
    connect(ui->QTabWidget_domain, SIGNAL( currentChanged(int) ),this, SLOT( change_domain(int)) );

    connect(ui->spinBox_max_iters, SIGNAL( valueChanged(int) ),this, SLOT( change_max_iters(int)) );
    connect(ui->doubleSpinBox_precision, SIGNAL( valueChanged(double) ),this, SLOT( change_precision(double)) );
    connect(ui->spinBox_py_start, SIGNAL( valueChanged(int) ),this, SLOT( change_star_pyramid(int)) );
    connect(ui->spinBox_py_step, SIGNAL( valueChanged(int) ),this, SLOT( change_step_pyramid(int)) );
    connect(ui->spinBox_py_stop, SIGNAL( valueChanged(int) ),this, SLOT( change_stop_pyramid(int)) );

    connect(ui->spinBox_rect_horizontal, SIGNAL( valueChanged(int) ),this, SLOT( change_rect_horizontal(int)) );
    connect(ui->spinBox_rect_vertical, SIGNAL( valueChanged(int) ),this, SLOT( change_rect_vertical(int)) );
    connect(ui->spinBox_ann_radial, SIGNAL( valueChanged(int) ),this, SLOT( change_ann_radial(int)) );
    connect(ui->spinBox_ann_angular, SIGNAL( valueChanged(int) ),this, SLOT( change_ann_angular(int)) );

    connect(ui->doubleSpinBox_rect_x0, SIGNAL( valueChanged(double) ),this, SLOT( change_rect_x0(double)) );
    connect(ui->doubleSpinBox_rect_xend, SIGNAL( valueChanged(double) ),this, SLOT( change_rect_xend(double)) );
    connect(ui->doubleSpinBox_rect_y0, SIGNAL( valueChanged(double) ),this, SLOT( change_rect_y0(double)) );
    connect(ui->doubleSpinBox_rect_yend, SIGNAL( valueChanged(double) ),this, SLOT( change_rect_yend(double)) );

    connect(ui->doubleSpinBox_ann_center_x, SIGNAL( valueChanged(double) ),this, SLOT( change_ann_center_x(double)) );
    connect(ui->doubleSpinBox_ann_center_y, SIGNAL( valueChanged(double) ),this, SLOT( change_ann_center_y(double)) );
    connect(ui->doubleSpinBox_ann_ro, SIGNAL( valueChanged(double) ),this, SLOT( change_ann_ro(double)) );
    connect(ui->doubleSpinBox_ann_ri, SIGNAL( valueChanged(double) ),this, SLOT( change_ann_ri(double)) );

    connect(ui->doubleSpinBox_blob_center_x, SIGNAL( valueChanged(double) ),this, SLOT( change_blob_center_x(double)) );
    connect(ui->doubleSpinBox_blob_center_y, SIGNAL( valueChanged(double) ),this, SLOT( change_blob_center_y(double)) );
    connect(ui->doubleSpinBox_blob_x_scale, SIGNAL( valueChanged(double) ),this, SLOT( change_blob_x_scale(double)) );
    connect(ui->doubleSpinBox_blob_y_scale, SIGNAL( valueChanged(double) ),this, SLOT( change_blob_y_scale(double)) );

    // Create Menu actions - Help
    connect(ui->action_About, SIGNAL( triggered() ), this, SLOT( about() ) );

    // Create Bottom actions
    connect(ui->pushButton_show_sets, SIGNAL( released() ), this , SLOT( reverse_sets() ));
    ...
    connect(manager, SIGNAL( send_und_inside_points ( v_points , bool ) ), und_imageLabel, SLOT( set_inside_points ( v_points, bool ) ) );
    connect(manager, SIGNAL( send_def_inside_points ( v_points , bool ) ), def_imageLabel, SLOT( set_inside_points ( v_points, bool ) ) );
...
void MainApp::change_Initial_Guess( int guess_in )
{
    ...
    if (initial_guess_iv == ic_User) def_imageLabel->selecting_ic = true;
    else def_imageLabel->selecting_ic = false;
...
{
    interpolation_iv = static_cast<interpolationModelEnum>( interpolation_model_in );
    subdivisions->interpolation_model_Box->setCurrentIndex( interpolation_iv );
}
...
{
    deformation_description_iv = static_cast<deformationDescriptionEnum>( deformation_description_in );
    ...
    switch ( processor_iv )
    {
        case processor_GPU:
        {
            ...
            else
            {
                ...
                processor_iv = processor_CPU;
                subdivisions->processorBox->setCurrentIndex(processor_iv);
                qobject_cast<QStandardItemModel*>( subdivisions->processorBox->model() )->item(processor_GPU)->setEnabled(false);

                QMessageBox::information(this, tr("MainWindow"),
                                         tr("There are no available device(s) that support CUDA\nDisabling GPU mode and continuing in CPU mode") );
            }
            #endif
            break;
        }
...
void MainApp::loadNxtGpuImages()
{
    ...
    if ( processor_iv == processor_GPU && preload_images_iv )
    {
        ...
        for ( int iImage = 2 ; iImage < fileNames.size() ; ++iImage )
        {
            ...
            cuda_manager->tempQ.push( cv::imread( fileNames.at( iImage ).toStdString() , color_flag ) );
            nvtxRangePop();
        }
    }
#endif
...
{
    rectangularDomain.x_center = (rectangularDomain.x_begin + rectangularDomain.x_end ) / 2.f;
    rectangularDomain.y_center = (rectangularDomain.y_begin + rectangularDomain.y_end ) / 2.f;
    ...
    printf("%15s %30s: center x = %6.2f  center_y = %6.2f  left_x = %6.2f  right_x = %6.2f  left_y = %6.2f  right_y = %6.2f  scale_x = %6.2f  scale_y = %6.2f\n",
           "MainApp","updateRectangularCenter", rectangularDomain.x_center, rectangularDomain.y_center,
           rectangularDomain.x_begin, rectangularDomain.x_end,rectangularDomain.y_begin,rectangularDomain.y_end,
           scale_x, scale_y);
#endif
}
...
void MainApp::change_blob_center_y( double yc_in )
{
    for ( int i = 0; i < (int) blobDomain.xy_contour.size(); ++i )
    {
        ...
        und_imageLabel->xy_blob[ i ].second = blobDomain.xy_contour[ i ].second * scale_y;
        def_imageLabel->xy_blob[ i ].second = blobDomain.xy_contour[ i ].second * scale_y;
    }
...
void MainApp::change_blob_x_scale( double xs_in )
{
    for ( int i = 0; i < (int) blobDomain.xy_contour.size(); ++i )
    {
        blobDomain.xy_contour[ i ].first = blobDomain.x_center +
                (blobDomain.xy_contour[ i ].first - blobDomain.x_center) *
                (float)xs_in / blobDomain.x_scale;
...
void MainApp::change_blob_y_scale( double ys_in )
{
    for ( int i = 0; i < (int) blobDomain.xy_contour.size(); ++i )
    {
        blobDomain.xy_contour[ i ].second = blobDomain.y_center +
                ( blobDomain.xy_contour[ i ].second - blobDomain.y_center ) *
                (float) ys_in / blobDomain.y_scale;
...
void MainApp::about()
{
    QMessageBox::about(this, tr("About Correlation V2.0"),
...
{    
    // Right layout
    color_mode_iv = static_cast<colorEnum>( subdivisions->colorBox->currentIndex() );
    ui->comboBox_color->setCurrentIndex(color_mode_iv);

    precision = subdivisions->precision_Box->value();
    ui->doubleSpinBox_precision->setValue(precision);

    deformation_description_iv = static_cast<deformationDescriptionEnum>( subdivisions->deformation_descriptionBox->currentIndex() );

    reference_image_iv = static_cast<referenceImageEnum>( subdivisions->reference_imageBox->currentIndex() );

    processor_iv = static_cast<processorEnum>( subdivisions->processorBox->currentIndex() );
    ...
    interpolation_iv = static_cast<interpolationModelEnum>( subdivisions->interpolation_model_Box->currentIndex() );
    ui->comboBox_interpolation_model->setCurrentIndex(interpolation_iv);
    ...
    {
        disconnect(manager, SIGNAL( display_images(QString, QString) ), this, SLOT( display_two_images(QString, QString) ) );
    }


    // far right layout
    error_handlingMode_iv = static_cast<errorHandlingModeEnum>( subdivisions->error_handlingModeBox->currentIndex() );
...
void MainApp::normalSize()
{
    ...
    //connect(this, SIGNAL(valueChanged_scaleSelection () ), und_imageLabel, SLOT( scaleSquare() ) );
    //connect(this, SIGNAL(valueChanged_scaleSelection () ), def_imageLabel, SLOT( scaleSquare() ) );
...
void MainApp::scaleImage(double factor)
{
    ...
    ui->scrollArea_und_image->setWidgetResizable( false );
    ui->scrollArea_def_image->setWidgetResizable( false );
    und_imageLabel->resize(und_imageLabel->realScale_x * und_imageLabel->pixmap()->width(), und_imageLabel->realScale_y * und_imageLabel->pixmap()->height());
    def_imageLabel->resize(def_imageLabel->realScale_x * def_imageLabel->pixmap()->width(), def_imageLabel->realScale_y * def_imageLabel->pixmap()->height());
    ...
    ui->actionZoom_in_25->setEnabled (und_imageLabel->realScale_x < 6.0   && und_imageLabel->realScale_y < 6.0 );
    ui->actionZoom_Out_25->setEnabled(und_imageLabel->realScale_x > 0.167 && und_imageLabel->realScale_y > 0.167 );
    ui->actionZoom_in_25->setEnabled (def_imageLabel->realScale_x < 6.0   && def_imageLabel->realScale_y < 6.0 );
    ui->actionZoom_Out_25->setEnabled(def_imageLabel->realScale_x > 0.167 && def_imageLabel->realScale_y > 0.167 );
}
...
void MainApp::enableRectangularCorrelate(float center_x_in, float center_y_in, float right_x_in, float right_y_in, float left_x_in, float left_y_in,
...
#if DEBUG_DOMAIN_SELECTION
    printf("%15s %30s: center x = %6.2f  center_y = %6.2f  left_x = %6.2f  right_x = %6.2f  left_y = %6.2f  right_y = %6.2f  scale_x = %6.2f  scale_y = %6.2f\n",
           "MainApp","enableRectangularCorrelate", rectangularDomain.x_center, rectangularDomain.y_center,
           rectangularDomain.x_begin, rectangularDomain.x_end,rectangularDomain.y_begin,rectangularDomain.y_end,
           scale_x, scale_y);
#endif
...
void MainApp::enableAnnularCorrelate(float center_x_in, float center_y_in, float inside_radius_in, float outside_radius_in, float ri_by_ro_in,
                              float scale_x_in, float scale_y_in)
{
    annularDomain.ready = true;
...
void MainApp::enableBlobCorrelate(v_points xy_blob_in,float scale_x_in, float scale_y_in)
{
    ...
    for ( int i = 0; i < (int)xy_blob_in.size(); ++i )
    {
        blobDomain.xy_contour.push_back( std::make_pair(
                                             xy_blob_in[ i ].first  / scale_x ,
                                             xy_blob_in[ i ].second / scale_y ) );
...
void MainApp::updateDomains()
{
    ...
    std::cout << std::endl;
    printf("%15s %30s: center x = %6.2f  center_y = %6.2f  left_x = %6.2f  right_x = %6.2f  left_y = %6.2f  right_y = %6.2f  scale_x = %6.2f  scale_y = %6.2f\n",
           "MainApp","updateDomains", und_imageLabel->center_x, und_imageLabel->center_y,
           und_imageLabel->left_x, und_imageLabel->left_y, und_imageLabel->right_x, und_imageLabel->right_y,
           scale_x, scale_y);
#endif
        ...
        case domain_annular:
            ...
            und_imageLabel->outside_radius = annularDomain.r_outside * scale_x; //Scale radius with x
        ...
        case domain_blob:
            ...
            if ( blobDomain.xy_contour.size() )
            {
                ...
                for ( int i = 0; i < (int) blobDomain.xy_contour.size(); ++i )
                {
                    ...
                    und_imageLabel->xy_blob.push_back( std::make_pair(
                                                           blobDomain.xy_contour[ i ].first  * scale_x,
    ...
    //connect(this, SIGNAL( valueChanged_domain() ), und_imageLabel, SLOT(GUIupdated() ) );
...
void MainApp::correlate()
{
    ...
    disconnect(ui->pushButton_correlate, SIGNAL( released() ), this, SLOT( correlate() ) );
    connect   (ui->pushButton_correlate, SIGNAL( released() ), this, SLOT( stop_correlate() ) );

    // Set up correlation parameters - first domain independent parameters and images
...
{
     number_of_model_parameters = ModelClass::get_number_of_model_parameters( model_iv );
     ...
     // Manager, und_imageLabel and def_imageLabel's initial guess are pointing to MainApp's initial_guess
...
void MainApp::correlation_done( bool error_in )
{
    ...
    // Once the correlation is done, re-enable the domain selection and saving analysis
    ui->action_Print->setEnabled                    ( true          );
    ...
    disconnect  ( ui->pushButton_correlate, SIGNAL( released() ), this, SLOT( stop_correlate() ) );
    connect     ( ui->pushButton_correlate, SIGNAL( released() ), this, SLOT( correlate()      ) );
...
{
    reportFile = QFileDialog::getSaveFileName(this,tr("Save Report to File"),"/home/namascar/Documents/data/reports");
    report.open( reportFile.toStdString() );
    report << manager->report.rdbuf();
    report.close();
}

void MainApp::stop_correlate()
{
...
{
    /**
     Method is called from within mainApp (without parameters) to indicate a new set of initial guess is available. It displays
...
void MainApp::initialize_domains()
{
    ...
    ui->doubleSpinBox_rect_xend->setValue( rectangularDomain.x_end                   );
    ui->doubleSpinBox_rect_yend->setValue( rectangularDomain.y_end                   );
    ui->spinBox_rect_horizontal->setValue( rectangularDomain.horizontal_subdivisions );
    ...
    //  Draw the initial annular domain outside the image. Setting ri = ro = 0 makes
